...
class sql4array
{
	...
	// gisanfu
	public function set_parse_select_as($bool = false)
	{
		$this->parse_select_as = $bool;
	}
	...
	protected function exec_query()
	{
		...
		foreach ($this->parse_from_as as $from_name => $table_name)
		{
			...
			foreach ($this->tables[$from_name] as $row)
			{
				...
				//echo $this->parse_where;
				//if (eval($this->parse_where))
				// gisanfu fix bug 雖然這個功能可以用，但是會報小錯
				if (@eval($this->parse_where))
				{
					// gisanfu fix
					if (isset($this->parse_select_as[0]) and $this->parse_select_as[0] == '*')
					{
