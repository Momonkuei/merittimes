...
class YiiBase
{
	 ...
	 * fix window separator symbol by lota 2017/11/26
	 */
	public static function getPathOfAlias($alias)
	{
		if(isset(self::$_aliases[$alias])){
			return str_replace('\\','/',self::$_aliases[$alias]);
		}
		elseif(($pos=strpos($alias,'.'))!==false)
		{
			$rootAlias=substr($alias,0,$pos);
			if(isset(self::$_aliases[$rootAlias]))
				return self::$_aliases[$alias]=str_replace('\\','/',rtrim(self::$_aliases[$rootAlias].DIRECTORY_SEPARATOR.str_replace('.',DIRECTORY_SEPARATOR,substr($alias,$pos+1)),'*'.DIRECTORY_SEPARATOR));
			...
			if(strpos($className,'\\')===false)  // class without namespace
			{
				if(self::$enableIncludePath===false)
				{
					foreach(self::$_includePaths as $path)
					{
						...
						if(@is_file($classFile)) // 將錯誤訊息隱藏 for 戰國策主機 by lota 2019/1/24
						{
							include($classFile);
