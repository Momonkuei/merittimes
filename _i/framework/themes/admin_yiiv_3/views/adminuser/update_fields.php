		...
		<?php if(!empty($v['field'])):?>
				<?php if($v['type'] == '1' or $v['type'] == '2'):?>
					<fieldset>
						...
						<table width="98%" border="0" cellspacing="1" cellpadding="7" class="table1">
						<tbody>
							<?php $this->data['k'] = $k?>
							<?php $this->data['v'] = $v?>
				...
				<?php // 合併簡易和進階授權表 ?>
				<?php if(preg_match('/^sub/', $this->data['router_class'])):?>
					<?php // 如果sub開頭，是不會顯示授權表的?>
				<?php else:?>
					<?php echo $this->renderPartial('//'.$this->data['router_class'].'/perm_merge', $this->data)?>
				<?php endif?>

				<?php if($v['type'] == 'xxxxxxx'):?>
					<dl class="inline">
					<?php $this->data['k'] = $k?>
					<?php $this->data['v'] = $v?>
					<?php echo $this->renderPartial('//default/update_fields_section', $this->data)?>
