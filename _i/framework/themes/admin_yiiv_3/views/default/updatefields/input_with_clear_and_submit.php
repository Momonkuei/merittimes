...
<?php
		$tmp = <<<XXX
$('#{$kk}_clear_and_submit').click(function(){
	$('#{$kk}').attr('value', '');
	$('#form_data').submit();
	return false;
});
...
<?php elseif(isset($this->data['vv_type_select']) and $this->data['vv_type_select'] == 'view'):?>
	...
	<a href="javascript:;" id="<?php if(isset($vv['attr']['id'])):?><?php echo $vv['attr']['id'].'_'?><?php endif?>clear_and_submit"><i class="icon-remove"></i></a>
