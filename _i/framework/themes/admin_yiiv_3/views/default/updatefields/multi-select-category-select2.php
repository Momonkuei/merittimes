...
<?php
	...
	if(!function_exists('judge_layer_g')){
		function judge_layer_g($pid = 0,$k = 0, $table_type){
			...
			if ($pid != 0){
				...
				//$rows = $cidb->select('id, pid')->where('is_enable',1)->where('ml_key',$_SESSION['auth_admin_data_ml_key'])->where('id',$pid)->get($table_type)->row_array();
				$rows = $cidb->select('id, class_id as pid')->where('is_enable',1)->where('ml_key',$_SESSION['auth_admin_data_ml_key'])->where('type',$table_type)->where('id',$pid)->get('html')->row_array();
				//echo $table_type;die;
	...
	$rows = $this->cidb->select('*, class_id as pid, class_id AS parent_id')->where('is_enable',1)->where('ml_key',$this->data['admin_switch_data_ml_key'])->where('type',$table_type)->order_by('sort_id')->get('html')->result_array();
	$tree = new Tree();
	$bbb = array();
	$current_row = array();
	$rows_ids = array();
	...
	if($rows){
		foreach($rows as $k => $v){
			...
			//判斷目前ID的層數
			$ggg = judge_layer_g($v['id'], 0, $table_type);
			$now_layer = $ggg['k'] + 1;
	...
	$str = "<option value='\$id'>\$spacer \$topic</option>";
