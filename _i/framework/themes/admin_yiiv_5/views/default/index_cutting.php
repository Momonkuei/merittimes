...
$script_default_a = <<<XXX0
	$('.del_button').click(function(){
		...
		if(confirm(l.get('Are you sure you want to delete') + ' ' + name + ' ? ')){
	...
	<div class="portlet-body" style="padding:0px">
		...
		<table id="tablelist" class="table1 sortable table table-striped table-bordered table-hover dataTable" aria-describedby="sample_1_info">
			...
			<tr>
				...
				<!--<th width="<?php if(isset($def['action_width'])):?><?php echo $def['action_width']?><?php else:?>10%<?php endif?>"><?php if(isset($def['action_name'])):?><?php echo $def['action_name']?><?php else:?><?php G::te($this->data['theme_lang'], 'Form Action', null, '動　作')?><?php endif?></th>-->
<?php //#33978?>
				<?php if(isset($def['sortable']['enable']) and $def['sortable']['enable'] == true and (($sort_field_nobase64 == $sort_id_tmp and $sort_direction == 'asc') or ($sort_field == '' and $sort_direction == '' and $def['default_sort_field'] == $sort_id_tmp))):?>
					<th width="<?php if(isset($def['action_width'])):?><?php echo $def['action_width']?><?php else:?>5%<?php endif?>"><?php if(isset($def['action_name'])):?><?php echo $def['action_name']?><?php else:?>排序<?php endif?></th>
				<?php endif?>

				<?php if(!isset($def['disable_edit']) or $def['disable_edit'] != true):?>
					<th width="<?php if(isset($def['action_width'])):?><?php echo $def['action_width']?><?php else:?>5%<?php endif?>"><?php if(isset($def['action_name'])):?><?php echo $def['action_name']?><?php else:?>編輯<?php endif?></th>

				<?php endif?>

				<?php if(!isset($def['disable_delete']) or $def['disable_delete'] != true):?>

					<th width="<?php if(isset($def['action_width'])):?><?php echo $def['action_width']?><?php else:?>5%<?php endif?>"><?php if(isset($def['action_name'])):?><?php echo $def['action_name']?><?php else:?>刪除<?php endif?></th>
...
<?php if(0):?>
			<tr class="<?php echo ($k%2==1)?'bgcolor1':'bgcolor2' ?>" id="sortable_id_<?php echo $v[$def['func_field']['id']]?>">
<?php endif?>
			<tr class="<?php echo ($k%2==1)?'even':'odd' ?>" id="sortable_id_<?php echo $v[$def['func_field']['id']]?>">
				...
				<?php if(!isset($def['disable_action']) or $def['disable_action'] != true):?>
					<?php //#33978?>
						...
						<?php if(isset($def['sortable']['enable']) and $def['sortable']['enable'] == true and (($sort_field_nobase64 == $sort_id_tmp and $sort_direction == 'asc') or ($sort_field == '' and $sort_direction == '' and $def['default_sort_field'] == $sort_id_tmp))):?>
<td>
							<a class="btn default btn-xs blue move" href="#" title="移動"><i class="icon-move"></i><?php G::te($this->data['theme_lang'], 'Move', null, '移動')?></a>
</td>
						<?php endif?>
...
<td>
							<?php if(isset($def['enable_edit_button_changeto_view_button']) and $def['enable_edit_button_changeto_view_button'] == true):?>
								...
								<a class="btn default btn-xs red-stripe" href="<?php echo $class_url?>/update&param=<?php echo $parameter['value'].$v[$def['func_field']['id']].'-'.$parameter['prev'].$current_base64_url?>" title="Edit"><?php G::te($this->data['theme_lang'], 'View', null, '檢視')?><</a>
							<?php else:?>
								<a class="btn default btn-xs purple" href="<?php echo $class_url?>/update&param=<?php echo $parameter['value'].$v[$def['func_field']['id']].'-'.$parameter['prev'].$current_base64_url?>" title="Edit"><i class="icon-edit"></i><?php G::te($this->data['theme_lang'], 'Edit', null, '編輯')?></a>
							<?php endif?>
</td>
							<?php $this->EndWidget('system.widgets.Gw_acl')?>
						<?php endif?>
...
<td>
							<a class="btn btn-xs red del_button" href="javascript:;" title="Delete" itemid="<?php echo $v[$def['func_field']['id']]?>" itemname="<?php if(isset($v['name']) or isset($v['topic'])) { echo (isset($v['name']))?strip_tags($v['name']):strip_tags($v['topic']); }?>"><i class="icon-trash"></i><?php G::te($this->data['theme_lang'], 'Delete', null, '刪除')?></a>
</td>
							<?php $this->EndWidget('system.widgets.Gw_acl')?>
						<?php endif?>
						...
						<?php if(isset($def['enable_view']) and $def['enable_view'] == true):?>
							...
							<a class="btn default btn-xs red-stripe" href="<?php echo $class_url?>/view&param=<?php echo $parameter['value'].$v[$def['func_field']['id']].'-'.$parameter['prev'].$current_base64_url?>"><?php G::te($this->data['theme_lang'], 'View', null, '檢視')?></a>
							<?php $this->EndWidget('system.widgets.Gw_acl')?>
						<?php endif?>
