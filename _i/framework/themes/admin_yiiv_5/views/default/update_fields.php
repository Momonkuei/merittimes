<?php if(!empty($def['updatefield']['sections'])):?>
	...
	<?php endforeach?>
	<?if($this->data['router_class']=='writeplan' && !empty($this->data['writeplanreview'])){?>
<hr>
<div class="portlet box light-grey">
		<div class="portlet-title">
			<div class="caption"><i class="icon-reorder"></i></div>
			<div class="tools">
				<a item_id="5" class="collapse" href="javascript:;"></a>
			</div>
		</div>
	<div class="portlet-body" style="overflow: hidden; display: block;">
		<table width="98%" border="0" cellspacing="1" cellpadding="7" class="table1 table">
			<tbody>
				<tr class="bgcolor1">
					<td width="160">
						新增修改紀錄<span class="required"></span>				
					</td>
				<td>
			<div class="View" style=""><table border="0">
				<tbody>
					<tr>
						<td>修改記錄</td>
						<td >使用者</td>
					...
					<?foreach($this->data['writeplanreview'] as $wk => $wv){?>
						<tr>
							<td><?=($wv['type']=='create'?'新增':'修改')?></td>
							<td><?=$wv['modifier']?></td>
							<td><?=$wv['a_results']?></td>
							<td><?=$wv['r_comments']?></td>
							<td><?=$wv['create_time']?></td>
						</tr>
					<?}?>
				</tbody>
			</div>
