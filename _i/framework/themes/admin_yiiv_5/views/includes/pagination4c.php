...
<?php 
if(!isset($pagination['current_start_record'])) $pagination['current_start_record'] = 0;
if(!isset($pagination['current_end_record'])) $pagination['current_end_record'] = 0;
$pagi_params = array(
...
?>
		...
		<?php if(isset($pagination['disable_current_record']) and $pagination['disable_current_record']):?>
			<?php G::te($this->data['theme_lang'], 'Page %d / %d', $pagi_params, '頁次 %d / %d')?>
		<?php else:?>
			<?php G::te($this->data['theme_lang'], 'Page %d / %d, Current Page by %d- %d, total $d', $pagi_params, '頁次 %d / %d, 本頁顯示 %d - %d 筆, 全部共 %d 筆')?>
		<?php endif?>

		<?php if(isset($pagination['disable_rows_per_page']) and $pagination['disable_rows_per_page']):?>
		<?php else:?>
, <?php G::te($this->data['theme_lang'], 'Rows per page', null, '每頁顯示筆數')?>
		<select class=" form-control" style="width:80px;display:inline" id="pagination4c_select" onchange="javascript:change_pagination4c_count();">
...
<?php endif?>
	...
	<div class="col-md-7 col-sm-12">
		<div class="dataTables_paginate paging_bootstrap pagination" style_x="margin:0;float:right">
			<ul class="pagination">
