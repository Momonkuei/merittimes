...
<tr>
		...
		<th><label title="打勾就是能用，反之"> &nbsp;<?php G::te(null, 'Simple', array(), '簡易') ?>&nbsp;</label></th>
		<th><label title="跟簡易一樣，但是可以細到例如能不能新增等">&nbsp;&nbsp;<?php G::te(null, 'Professional Permission', array(), '進 階 授 權') ?></label></th>
</tr>
</thead>
<tbody>
		...
		<?php foreach($resources as $k => $resource):?>
			...
			<td>
				<?php foreach($resource['actions'] as $action):?>
					<label>
						<span>
							<input type="checkbox" <?php if(isset($autzs2[$resource['name']][$action]) and $autzs2[$resource['name']][$action] > 0): ?>checked="checked"<?php endif; ?> value="1" name="autz2[<?php echo $resource['name']?>][<?php echo $action ?>]" stylex="opacity: 1;">
							</span>
							<?php if($action == 'index'):
		...
		<?php foreach($urls as $k => $url):?>
			...
			<td>
				<?php foreach($url['actions'] as $action):?>
					<label>
						<span>
							<input type="checkbox" <?php if(isset($autrs2[$url['id']][$action]) and $autrs2[$url['id']][$action] > 0): ?>checked="checked"<?php endif; ?> value="1" name="autr2[<?php echo $url['id']?>][<?php echo $action ?>]" stylex="opacity: 1;">
							</span>
							<?php if($action == 'index'):
								G::te(null, $action, array(), '列表');
							elseif($action == 'create'):
