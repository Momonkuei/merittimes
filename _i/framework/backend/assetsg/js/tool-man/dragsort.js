...
ToolMan._dragsortFactory = {
	...
	// 指定的位置拖拉
	makeSortable2 : function(item, item2) {
		var group = ToolMan.drag().createSimpleGroup(item, item2)
	...
	makeListSortable : function(list) {
		...
		helpers.map(items, function(item) {
			//var idname = item.id;
			//alert(idname);
			//var dragGroup = dragsort.makeSortable2(item, document.getElementById(idname + '_dragarea'))
			//var dragGroup = dragsort.makeSortable(item)

			//if(idname != '' && typeof(document.getElementById(idname + '_dragarea')) !== 'undefined' && document.getElementById(idname + '_dragarea') != null && document.getElementById(idname + '_dragarea').length != "undefined"){
			//	var dragGroup = dragsort.makeSortable2(item, document.getElementById(idname + '_dragarea'))
			//	//var dragGroup = dragsort.makeSortable(document.getElementById(idname + '_dragarea'))
			//} else {
			//	var dragGroup = dragsort.makeSortable(item)
			//}
			var dragGroup = dragsort.makeSortable(item)
