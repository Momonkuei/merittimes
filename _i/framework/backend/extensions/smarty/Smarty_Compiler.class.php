...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_capture_tag($start, $tag_args = '')
    {
        ...
        if ($start) {
            ...
            // gisanfu
            // 主要是給capture_head這個自定變數陣列元素名稱所使用
            $var_name = isset($attrs['varname']) ? $attrs['varname'] : 'capture';
            
            $output = "<?php ob_start(); ?>";
            $this->_capture_stack[] = array($buffer, $assign, $append, $var_name);
        } else {
            list($buffer, $assign, $append, $var_name) = array_pop($this->_capture_stack);
            // gisanfu mark
            //$output = "<?php \$this->_smarty_vars['capture'][$buffer] = ob_get_contents(); ";
            //$output = '<?php if(!isset($this->_smarty_vars['.$var_name.']['.$buffer.'])){ $this->_smarty_vars['.$var_name.']['.$buffer.'] = ob_get_contents(); }';

            // 原本的
            $output = '<?php $this->_smarty_vars['.$var_name.']['.$buffer.'] = ob_get_contents();';
            if (isset($assign)) {
    ...
    function _compile_smarty_ref(&$indexes)
    {
        ...
        switch ($_ref) {
            ...
            // gisanfu 在head區段輸出，通常是capture_head的上面
            case 'capture_head_load':
                return null;
            ...
            // gisanfu 在body結束之前輸出，通常是capture的上面
            case 'capture_load':
                return null;

            // gisanfu 不會在任何區塊輸出，自由使用
            case 'capture_other':
                return null;
