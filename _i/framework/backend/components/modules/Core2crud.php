...
class XXXXXXXX_A extends XXXXXXXX_B
{
	...
	protected function default_def()
	{
		return array(
			'header_closed' => false, // 
			'page_sidebar_closed' => false, // 側邊選單
			...
			'index_buttons' => array(
				// array(
				// 	'name' => '我是按鈕<i class="icon-plus"></i>',  // 按鈕的名稱和圖示
				// 	'name2' => 'newfunc', // 假設create，那權限也是用create，那該功能也要開create(admin_resource)，雖然create早就有了，這裡只是範例而以
				// 	'id' => '', // button
	...
	public function actionIndex($param = '')
	{
		...
		$this->index_get_data();
		$this->index_last_handle(); // 2020-08-17 如果要客製列表欄位，記得在這個之前
	...
	protected function index_param_handle()
	{
		...
		if($this->data['params']['sort'] == ''){
			...
			if(isset($this->data['def']['default_sort_field'])){
				...
				$sort_field = $this->data['func_field']['id'];
			}
		} else {
		...
		if($this->data['params']['search'] != ''){
			...
			if(isset($search_tmp[0]) and $search_tmp[0] == $this->data['router_class'] and $search_tmp[1] != ''){
	...
	protected function index_get_total()
	{
		...
		if($this->data['search_keyword'] != '' and isset($this->data['def']['search_keyword_field']) and !empty($this->data['def']['search_keyword_field'])){
	...
	public function actionUpdate($param = '')
    {
        if(empty($_POST)){
			...
			// 2021-03-22
			$validation = $this->update_show_validation($validation);
			...
			// #46369 社群分享--防止其他資料被修改掉
			if($this->data['router_class']=='community'){
				unset($update['class_ids']);
				unset($update['file']);
				unset($update['pid']);
				unset($update['pic1___origin']);
				// print_r($update);
				// die;
			}
			//var_dump($update);
			...
			if($this->data['router_class']=='writeplan'){
				if(isset($update['a_results']) && ($update['a_results']=='核可' || $update['a_results']=='未核可')){
					$update['review_time']=date('Y-m-d H:i:s');
				}
				//審核紀錄 有修改審核結果 or 回覆意見才更新紀錄
				if(isset($update['a_results']) &&  ($old_u['a_results']!=$update['a_results'] || $old_u['r_comments']!=$update['r_comments'])){
					$update['modifier']=$_SESSION['auth_admin_name'];
					/* 審核紀錄增加 */
					$data_num=$this->cidb->where('class_id',$id)->get('writeplanreview')->num_rows();
					if($data_num==0){
						$array=array(
							'type'=>'create',
							'class_id'=>$id,
							'a_results'=>$update['a_results'],
							'r_comments'=>$update['r_comments'],
							'modifier'=>$_SESSION['auth_admin_name'],
					...
					$this->cidb->insert('writeplanreview', $array);
				}				
			}
			...
			$update['update_base64_url']="";//20220913 lin 增加使頁面回到列表頁
...
// AAAAAAAAAA
	...
	protected function update_run_pic($update, $old_u , $lid = 0)
	{
		...
		if($lid!=0){
			...
			//if(!isset($update[$image_fieldname]) or $update[$image_fieldname] == '') continue;//2016/7/1 eval有問題，先註解
		...
		for($x=1;$x<=10;$x++){
			...
			if($update[$image_fieldname] != $old_u[$image_fieldname] && $update[$image_fieldname]!=''){
				$copy_source = $this->data['image_upload_tmp_path'].'/'.$update[$image_fieldname];
				if(file_exists($copy_source)){
					// 自訂路徑修正
					if(isset($this->data['def']['pic_upload_path']) and $this->data['def']['pic_upload_path'] != ''){
		 ...
		 * }
		 */
		if(isset($this->data['def']['multiyoutube_upload']) and !empty($this->data['def']['multiyoutube_upload'])){
			...
			foreach($this->data['def']['multiyoutube_upload'] as $k => $v){
				 ...
				 * }
				 */
				if(isset($update['youtubes'][$k]) and !empty($update['youtubes'][$k])){
					foreach($update['youtubes'][$k] as $kk => $vv){
						...
						// 讀取額外的屬性與值
						if(isset($update['youtubes_attr'][$k]) and !empty($update['youtubes_attr'][$k])){
						...
						// 讀取上傳後額外的屬性值
						if(isset($update['youtubes_fileattr'][$k]) and !empty($update['youtubes_fileattr'][$k])){
		 ...
		 *  }
		 */
		if(isset($this->data['def']['multifile_upload']) and !empty($this->data['def']['multifile_upload'])){
			$update_productpic = array();
			// @k 設定名稱
			// @v 屬性群
			foreach($this->data['def']['multifile_upload'] as $k => $v){
				...
				// 如果有資料，那就準備要寫入了
				if(isset($update['uploads'][$k]) and !empty($update['uploads'][$k])){
					foreach($update['uploads'][$k] as $kk => $vv){
						...
						// 讀取額外的屬性與值
						if(isset($update['uploads_attr'][$k]) and !empty($update['uploads_attr'][$k])){
			...
			foreach($this->data['def']['updatefield']['sections'] as $k => $v){
				if(isset($v['field'])){
					...
					foreach($v['field'] as $kk => $vv){
						...
						if($vv['type'] == 'treeselect'){
							// 資料格式是這樣子 ,1,2,3,
							$rows = $this->data['updatecontent'][$kk];
							$results = array();
							if($rows and !empty($rows)){
								foreach($rows as $kkk => $vvv){
									if(isset($vvv['is_selected']) and $vvv['is_selected'] == '1'){
										$results[] = $vvv['name'];
									}
								}
							}
							$this->data['updatecontent'][$kk] = implode(',', $results);
							$this->data['def']['updatefield']['sections'][$k]['field'][$kk]['type'] = 'label';
						} elseif($vv['type'] == 'select3'){
	...
	public function actionCreate($param = '')
    {
        if(empty($_POST)){
			...
			// 2020-04-13 (請參照本檔案的2015-09-24上面的說明)
			// https://redmine.buyersline.com.tw/issues/35403
			unset($this->def['updatefield']['sections'][0]['field']['sort_id']);
			unset($this->def['updatefield']['sections'][1]['field']['sort_id']);

			$this->data['def'] = G::definit($this->def, $this->data);
			$this->data['params'] = $params;
			$this->data['parameter'] = $param_define;

			//unset($this->data['def']['updatefield']['sections'][0]['field']['sort_id']);
			//unset($this->data['def']['updatefield']['sections'][1]['field']['sort_id']);
			...
			if(isset($this->data['search_keyword']) and $this->data['search_keyword'] != ''){
				...
				// 如果要帶入多個欄位的話
				if(isset($this->data['def']['search_keyword_assign_field']) and is_array($this->data['def']['search_keyword_assign_field']) and !empty($this->data['def']['search_keyword_assign_field'])){
			...
			//$savedata = $this->replace_quotes($savedata); //2017/10/13 加入特殊字元處理 by lota 
			...
			//2023/01/03  用到date1欄位 未輸入 帶入當下日期 ps.類似最新消息單元等
			if(isset($savedata['date1']) && empty($savedata['date1'])){
				$savedata['date1']=date('Y-m-d');
			}
...
// AAAAAAAAAA
	...
	protected function create_show_validation($validation)
	{
...
// AAAAAAAAAA
	...
	protected function create_run_pic($savedata)
	{
		 ...
		 *  }
		 */
		if(isset($savedata['youtubes']) and !empty($savedata['youtubes'])){
			...
			if(isset($this->data['def']['multiyoutube_upload']) and !empty($this->data['def']['multiyoutube_upload'])){
				...
				foreach($this->data['def']['multiyoutube_upload'] as $k => $v){
					...
					// 如果有資料，那就準備要寫入了
					if(isset($savedata['youtubes'][$k]) and !empty($savedata['youtubes'][$k])){
						foreach($savedata['youtubes'][$k] as $kk => $vv){
							...
							$sort_id_num++;
							// 讀取額外的屬性與值
							if(isset($savedata['youtubes_attr'][$k]) and !empty($savedata['youtubes_attr'][$k])){
		 ...
		 *  }
		 */
		if(isset($savedata['uploads']) and !empty($savedata['uploads'])){
			$savedata_productpic = array();
			if(isset($this->data['def']['multifile_upload']) and !empty($this->data['def']['multifile_upload'])){
				// @k 設定名稱
				// @v 屬性群
				foreach($this->data['def']['multifile_upload'] as $k => $v){
					...
					// 如果有資料，那就準備要寫入了
					if(isset($savedata['uploads'][$k]) and !empty($savedata['uploads'][$k])){
						foreach($savedata['uploads'][$k] as $kk => $vv){
							...
							$sort_id_num++;
							// 讀取額外的屬性與值
							if(isset($savedata['uploads_attr'][$k]) and !empty($savedata['uploads_attr'][$k])){
	...
	public function actionDelete($param = '')
    {
		...
		$u = $c::model()->exists($this->data['def']['func_field']['id'].'=:id',array(':id'=>$id));
		//echo $id;
...
// AAAAAAAAAA
	...
	// 2018-08-29 節點
	// parent必需使用id當做索引
	// child不限制class_id欄位，但一定要是數字欄位
	public function actionNode($param)
	{
		// param={value 索引值}__{parent router_class}__{child router_class}__{child field_name}
		$tmps = explode('__', $param);
		$value = $tmps[0];
		$parent = $tmps[1];
		$child = $tmps[2];
		$field = $tmps[3];
		...
		// $session['child'] = $child; // 這個不需要
		$session['field'] = $field;
		Yii::app()->session[$ss] = $session;

		$this->redirect($this->createUrl($child.'/index'));
	}

	public function actionDropdown_listcontent_trigger()
	{
        if(!empty($_POST)){
			$this->data['def'] = G::definit($this->def, $this->data);
			...
			if(isset($_POST['uptime'])){
				$is_uptime = $_POST['uptime'];//lota add
			}
			...
			if($u->update()){
				//lota add 紀錄修改的時間 ，記得要在相對應的資料表去新增欄位，不然會無動作 2016/7/19
				if(isset($is_uptime) and $is_uptime){
					if($value){
				...
				$sys_log_msg = 'dropdown_listcontent_trigger id:'.$id.', field:'.$field.', value:'.$value;
				sys_log::set($sys_log_msg);
	...
	public function actionCheckbox_listcontent_trigger()
	{
        if(!empty($_POST)){
			...
			if($u->update()){
				$this->checkbox_listcontent_trigger_last(); //2019/6/19 lota add
				//lota add 紀錄修改的時間 ，記得要在相對應的資料表去新增欄位，不然會無動作 2016/7/19
				if(isset($is_uptime) and $is_uptime){
					if($value){
						$u->{'update_time'} = date('Y-m-d H:i:s');
				...
				$sys_log_msg = 'checkbox_listcontent_trigger id:'.$id.', field:'.$field.', value:'.$value;
				sys_log::set($sys_log_msg);
	...
	protected function checkbox_listcontent_trigger_last()
	{
...
// AAAAAAAAAA
	...
	public function actionEzdelete($param = '')
	{
		...
		if(empty($_POST)){
			...
			foreach($ids as $k => $id){
				...
				if($u){
					$this->data['id'] = $id; //2019-6-19 lota add
					// 為了要回去上一頁
					$prev_url = base64url::decode($params['prev']);
	...
	public function actionReview($param = '')
	{
		...
		if(empty($_POST)){
			...
			if(empty($_POST) or count($_POST) <= 0){
				$msg = $this->_getLabel('Please Select');
				G::m($msg);
			}
			$ids = $_POST['ezdeletes'];
			$a_results=$_POST['a_results_f'];
			$r_comments=$_POST['r_comments_f'];
			foreach($ids as $k => $id){
				if($a_results!='核可'){
					$writeplan_data['apply_num']=0;
				}
				$array=array(
					'a_results'=>$a_results,
					'approved_num'=>$writeplan_data['apply_num'],
				...
				$this->cidb->update('writeplan',$array); 
				/* 修改紀錄 */
				$data_num=$this->cidb->where('class_id',$id)->get('writeplanreview')->num_rows();
				if($data_num==0){
					$array=array(
	...
	public function actionSort()
	{
		...
		if($new_sort_id == ''){
			...
			$c->select($sort_field);
			$c->from($table_name);
			$c->where($this->def['func_field']['id'].'='.$new_id);
			$row = $c->queryRow();
		...
		$fieldsorter->setIdName($this->def['func_field']['id']);
		// table_name, field_id, new_sort_id
	...
	public function actionExcelexport()
	{
		...
		// 2020-02-11
		if(isset($this->def['updatefield']['sections'][1])){
		...
		if($admin_field and !empty($admin_field)){
			...
			foreach($admin_field as $k => $v){
				...
				$objPHPExcel->getActiveSheet()->SetCellValue(PHPExcel_Cell::stringFromColumnIndex($i).'1', $v['label']);
				$i++;
			}
		}
			...
			foreach($rows as $k => $v){
				if($admin_field and !empty($admin_field)){
					...
					foreach($admin_field as $kk => $vv){
						...
						// $objPHPExcel->getActiveSheet()->SetCellValue(chr($i+65).($k+2), (string)$value,PHPExcel_Cell_DataType::TYPE_STRING); // 2019-05-08 發現超過26個欄位，就會失敗
						// $objPHPExcel->getActiveSheet()->SetCellValue(PHPExcel_Cell::stringFromColumnIndex($i).($k+2), (string)$value,PHPExcel_Cell_DataType::TYPE_STRING);
						...
						if(preg_match('/^pic/', $kk)){
							...
							//$objDrawingPType->setName("testggg");
							...
							// $objPHPExcel->getActiveSheet()->SetCellValue(chr($i+65).($k+2), '');
							// $objPHPExcel->getActiveSheet()->getColumnDimension(chr($i+65))->setWidth(22);

							$objPHPExcel->getActiveSheet()->SetCellValue(PHPExcel_Cell::stringFromColumnIndex($i).($k+2), '');
							$objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setWidth(22);
	...
	public function actionCsvexport()
	{
		...
		$objPHPExcel->getProperties()->setCreator("Buyersline");
		$objPHPExcel->getProperties()->setLastModifiedBy("Buyersline");
		$objPHPExcel->getProperties()->setTitle("CSV Product Document");
		$objPHPExcel->getProperties()->setSubject("CSV Product Document");
		$objPHPExcel->getProperties()->setDescription("Product Document for CSV");
			...
			foreach($rows as $k => $v){
				if($admin_field and !empty($admin_field)){
					...
					foreach($admin_field as $kk => $vv){
						...
						} elseif($kk == 'phone'){ // add phone format by lota
							$value = L::format_cellphone($value);
						}
						$objPHPExcel->getActiveSheet()->SetCellValue(PHPExcel_Cell::stringFromColumnIndex($i).($k+2), $value);
						...
						if(preg_match('/^pic/', $kk)){
							...
							//$objDrawingPType->setName("testggg");
							$objDrawingPType->setPath($this->data['image_upload_path'].'/'.$this->data['router_class'].'/'.$value);
							$objDrawingPType->setCoordinates(PHPExcel_Cell::stringFromColumnIndex($i).($k+2));
							$objDrawingPType->setHeight(100);
							$objPHPExcel->getActiveSheet()->getRowDimension($k+2)->setRowHeight(100);
							$objPHPExcel->getActiveSheet()->SetCellValue(PHPExcel_Cell::stringFromColumnIndex($i).($k+2), '');
							$objPHPExcel->getActiveSheet()->getColumnDimension(PHPExcel_Cell::stringFromColumnIndex($i))->setWidth(22);
		...
		$file = trim(str_replace('　','',$this->data['main_content_title'])).'-'.date('Y-m-d-H-i-s').'.csv';
		if(preg_match('/(MSIE|Trident)/', $_SERVER['HTTP_USER_AGENT'])){
			$file = iconv('utf-8','big5',$file);
		}
		header('Content-Disposition: attachment;filename="'.$file.'"');
		//header('Content-Disposition: attachment; filename='.trim(str_replace('　','',$this->data['main_content_title'])).'-'.date('Y-m-d-H-i-s').'.csv' ); 
	...
	protected function multi_field_v1($groupname, $tablename, $fields_pipe, $require_field, $section_number ,$sample_num = 5)
	{
		...
		if(!empty($_POST)){
			$update = $_POST;
			$update = $this->replace_quotes($update);//2016/5/31 fix double quotes for lota
			...
			if(!empty($membercontacts)){
				$_error = false; //設定錯誤旗標，只有其中一個錯就是true 2020-12-10 by lota
				foreach($membercontacts as $k => $v){
					foreach ($v as $k1 => $v1) {
						if(isset($v[$require_field]) and ($v[$require_field] == '' or $v[$require_field] == '0')){
							if($k1!='inventory'){ //這個目前在外面是寫死的...暫時先這樣處理
								if(!empty($v1)){		
									$_error = true;
									//$_error .=$k1.'<br/>';							
								}
		...
		// $data_tmp['def'] = $this->data['def']['updatefield']['sections'][$section_number]['field'][$groupname]['def']; // 2017-08-21 發現疑似Bug
		$data_tmp['def']['updatefield'] = $this->data['def']['updatefield']['sections'][$section_number]['field'][$groupname]['def']['updatefield'];
	...
	//搜尋取代雙引號 2016/6/7 加入指定處理欄位 //2017/10/13 加入特殊字元處理
	protected function replace_quotes($data = array(),$filed='')
	{
		if(is_array($data) && !empty($data)){
			foreach($data as $k => $v){
				...
				if($k=='detail' || $k=='field_data' || $k=='field_tmp'){
					$data[$k] = $this->numeric_code_utf8($v);
				}else{
					$data[$k] = $this->replace_quotes($v);
				}
			}
			return $data;
		}else{
			return str_replace('"','＂',$this->numeric_code_utf8($data));
		}
	}
	...
	/*
	 * 2018-02-14
	 * 這裡面的程式碼是從V1第二版裡面的dom5/layer.php拉出來的
	 * 很實用，會把帽號和逗點分隔的字串，透過json轉成陣列
	 */
	protected function string_to_json_decode_to_array($string = '')
	{
		if(trim($string) == ''){
			return false;
		}
		...
		return json_decode('{'.implode(',', $m_params2).'}', true);
	}
