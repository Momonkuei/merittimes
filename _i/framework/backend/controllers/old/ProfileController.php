...
XXX;
			...
			$rows = $this->db->createCommand()->from('admin_user')->where('is_enable=1 and id='.$this->data['admin_id'])->queryAll();
			...
			$row = $this->db->createCommand()->from('member')->where('is_enable=1 and login_password="'.sha1($update['old_login_password']).'"')->queryRow();
