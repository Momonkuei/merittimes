...
class Funcfieldv2updateController extends Controller {
	...
	protected $def = array(
		...
		'sortable' => array(
			...
			'url' => 'backend.php?r=funcfieldv2update/sort', // ajax post都會有個目標
		),
		// 建立前端要顯示的欄位
	...
	protected function beforeAction($action)
	{
		...
		$rows = $this->db->createCommand()->from('sys_func_v2')->where('is_enable=1')->order('sort_id asc')->queryAll();
		if($rows and count($rows) > 0){
			foreach($rows as $k => $v){
				if($k == 0 and (!isset($_SESSION['funcv2_editfield_id_updateX']) OR $_SESSION['funcv2_editfield_id_updateX'] == '')){
					$_SESSION['funcv2_editfield_id_updateX'] = $v['id'];
					$this->data['updatecontent']['func'] = $v['id'];
				}
				$this->def['searchfield']['sections'][0]['field']['func']['other']['values'][$v['id']] = $v['name'].'('.$v['func'].')';
			}
		}

		// 無法帶入的變數中的變數，在這裡帶入
		$this->def['condition'][0][0] = 'where';
		//$this->def['condition'][0][1] = 'is_enable=1 AND update_section=1 AND data_id=\''.$_SESSION['funcv2_editfield_id_update1'].'\' ';
		//$this->def['sortable']['condition'] = ' is_enable=1 AND update_section=1 AND data_id="'.$_SESSION['funcv2_editfield_id_update1'].'"';
		...
		$this->data['updatecontent']['func'] = $_SESSION['funcv2_editfield_id_updateX'];

		//$condition = 'is_checkout = 0 ';
		//$condition = 'is_enable = 1 ';
		$condition = ' is_enable = 1 AND data_id=\''.$_SESSION['funcv2_editfield_id_updateX'].'\' ';
		$condition2 = ' is_enable = 1 AND data_id="'.$_SESSION['funcv2_editfield_id_updateX'].'" ';
		if(isset($session) and count($session) > 0){
			...
			foreach($session as $k => $v){
				...
				if($k == 'class_id'){
					$conditions[] = 'update_section='.$v;
					$conditions2[] = 'update_section='.$v;
				} elseif($k == 'func'){
					$_SESSION['funcv2_editfield_id_updateX'] = $v;
					$this->data['updatecontent']['func'] = $v;
				} else {
					$conditions[] = $k.' LIKE \'%'.$v.'%\'';
				}
				//var_dump($conditions);
			...
			if(count($conditions2) > 0){
				...
				$condition2 .= implode(' AND ', $conditions2);
			}
			if($condition != ''){
				$this->def['sortable']['condition'] = $condition2;
			}
		} else {
			if(trim($condition) != ''){
	...
	{   
		...
		$row = $this->db->createCommand()->from('sys_func_v2')->where('id='.$_SESSION['funcv2_editfield_id_updateX'])->queryRow();
	...
	{
		$array['data_id'] = $_SESSION['funcv2_editfield_id_updateX'];
		return $array;
	}
	...
	protected function update_run_last()
	{
		...
		$_POST['updateother01']['from_user_id'] = (isset($_SESSION['authw_admin_id']))?$_SESSION['authw_admin_id']:'form1';
		$this->multi_field_v1('updateother01', 'sys_func_v2_update_other', 'keyname|keyvalue', 'keyname', 2);
