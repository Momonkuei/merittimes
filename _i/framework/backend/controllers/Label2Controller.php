...
class Label2Controller extends Controller {
	...
	protected $def = array(
		...
		'listfield_attr' => array(
			'smarty_include_top' => 'label2/main_content_top.htm',
		),
		// 建立前端要顯示的欄位
		'listfield' => array(
			'xxx' => array(
		...
		'updatefield' => array(
			...
			'smarty_javascript' => 'label2/update_javascript.htm',
			'method' => 'update',
			'form' => array(
				'enable' => true,
				'attr' => array(
	...
	//		)
	//	);
	//	$u = new Empty_orm('insert', $empty_orm_data_a);
	//	$data_a = array(
	//		'aaa' => 'test',
	//	);
	//	$u->setAttributes($data_a);
	//	if(!$u->save()){
	//		G::dbm($u->getErrors());
	//	}
	...
	//		)
	//	);
	//	$u = new Empty_orm('insert', $empty_orm_data_b);
	//	$data_b = array(
	//		'bbb' => 'test2',
	//	);
	//	$u->setAttributes($data_b);
	//	if(!$u->save()){
	//		G::dbm($u->getErrors());
	//	}
	...
	public function actionAutocomplete()
	{
		...
		foreach($results as $k => $v){
			...
			$tmps = array();
			foreach($v as $kk => $vv){
				if(isset($this->data['mls_frontend'][$kk])){
					$tmps[] = $this->data['mls_frontend'][$kk].'==='.$vv;
				}
			}
			$tmp .= implode('*****', $tmps);
			$return .= $tmp."\n";
	...
	protected function index_get_total()
	{
		...
		$sql = 'SELECT COUNT(*) AS count 
			...
			AND ml_label.key LIKE "[[web]] '.$this->data['search_keyword'].'%"
			AND ( ml_lang.label_key LIKE "[[web]] '.$this->data['search_keyword'].'%" OR ml_lang.`value` LIKE "%'.$this->data['search_keyword'].'%")';
	...
	{
		$this->data['def']['listfield']['value']['label'] = $this->data['mls_frontend'][$this->data['mls_frontend_tmp'][0]];
		...
		//$sql .= 'WHERE (ml_lang.ml_key="'.$this->data['mls_frontend_tmp'][0].'" OR (ml_lang.value IS NULL AND ml_lang.ml_key IS NULL)) ';
		$sql .= 'WHERE (ml_lang.ml_key="'.$this->data['ml_key'].'" OR (ml_lang.value IS NULL AND ml_lang.ml_key IS NULL)) ';
		$sql .= ' AND ml_label.key LIKE "[[web]] '.$this->data['search_keyword'].'%" ';
	...
	protected function update_show_last($updatecontent)
	{
		...
		// 	),
		// ),
		if(isset($this->data['mls_frontend']) and count($this->data['mls_frontend']) > 0){
			foreach($this->data['mls_frontend'] as $k => $v){
				$array = array(
					'label' => $v,
					'type' => 'inputx',
					'attr' => array(
...
*/
		...
		if(count($langs) > 0){
			foreach($langs as $k => $v){
				$g = $this->db->createCommand()->from('ml_lang')->where('label_key=:key and ml_key=:ml_key', array(':key'=>$label_key, ':ml_key'=>$k))->queryRow();
				if($g){
					$c = new Empty_orm('insert', $this->def['empty_orm_data_lang']);
					$u = $c::model()->findByPk($g['id']);
					$u->value = $v;
					if(!$u->update()){
						G::dbm($u->getErrors());
					...
					$u = new Empty_orm('insert', $this->def['empty_orm_data_lang']);
					$u->ml_key = $k;
					$u->label_key = $label_key;
					$u->value = $v;
					if(!$u->save()){
