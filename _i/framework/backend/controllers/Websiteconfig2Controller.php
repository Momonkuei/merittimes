...
class Websiteconfig2Controller extends Controller
{
			...
			'sections' => array(
				...
				array(
					...
					'field' => array(
						...
						'sys_config_seo_title' => array(
							'label' => 'SEO網站標題',
							'type' => 'input',
							'attr' => array(
								'id' => 'sys_config_seo_title',
								'name' => 'sys_config_seo_title',
								'title' => 'SEO使用',	
								'size' => '80',							
							),
						),
	...
	protected function beforeAction($action)
	{
		...
		if(defined('SEO_OPEN') && SEO_OPEN == true){
			...
			if($acl->hasAcl($this->data['admin_id'], 'seo', 'update')){
				...
				unset($this->def['updatefield']['sections'][0]['field']['sys_config_seo_title']);	
			}
		}else{
			unset($this->def['updatefield']['sections'][0]['field']['sys_config_has_seo']);
			unset($this->def['updatefield']['sections'][0]['field']['sys_config_seo_keyword']);
	...
	public function actionIndex($param='')
	{
		if(empty($_POST)){
			...
			if(SEO_OPEN == false){
				unset($list['has_seo']);
				unset($list['seo_keyword']);
				unset($list['seo_description']);
				unset($list['seo_title']);	
			}else{
				$acl = new Admin_acl();
				$acl->start();
			...
			if(count($list) > 0){
				foreach($list as $k => $v){
					...
					if(!preg_match('/^(admin|service|smtp|has_seo)/', $v)){
						$v .= '_'.$this->data['admin_switch_data_ml_key'];
					}
