...
//樣板選擇
if(isset($_SESSION['now_layout'])) $now_layout= $_SESSION['now_layout'];

//2016/1/25 lota 加入動態更換首頁參數  $layout_v2_indexname (一樣也只判斷子站)
if(isset($GLOBALS['lay_out_select_control']) && $this->data['router_method']=='index'){
	$layout_v2_name_lay = $this->data['router_class'].'-'.$this->data['router_method'].$GLOBALS['lay_out_select_control'];
	$router_method = $this->data['router_method'];
	$this->data['router_method'] = $this->data['router_method'].$GLOBALS['lay_out_select_control'];
}else
	$layout_v2_name_lay = $this->data['router_class'].'-'.$this->data['router_method'];
...
if(file_exists(Yii::getPathOfAlias('application.controllers.'.$layout_folder).'/first_'.$layout_v2_name.'.php')){
	$first_layout_v2_file = Yii::getPathOfAlias('application').'/controllers/'.$layout_folder.'/first_'.$layout_v2_name.'.php';
} else {
	$first_layout_v2_file = Yii::getPathOfAlias('system.frontend.controllers.'.$layout_folder).'/first_'.$layout_v2_name.'.php';
}
...
// 看有沒有layoutv2 結構檔
if(file_exists(Yii::getPathOfAlias('application.controllers.'.$layout_folder).'/layout_'.$layout_v2_name_lay.'.php')){
	$layout_v2_file = Yii::getPathOfAlias('application').'/controllers/'.$layout_folder.'/layout_'.$layout_v2_name_lay.'.php';
} else {
...
if(file_exists($layout_v2_file)){
	...
	//2016/1/28 lota 回復原始值
	if(isset($router_method)) $this->data['router_method'] = $router_method;
	...
	//} elseif(isset($_GET['save']) and $_GET['save'] == '1' and isset($this->data['admin_id']) and $this->data['admin_id'] != ''){ // 存檔於本地
		...
		$layout_v2_file = Yii::getPathOfAlias('application').'/controllers/'.$layout_folder.'/layout_'.$layout_v2_name.'.php';
		file_put_contents($layout_v2_file, '<'.'?'.'php $layouts = '.var_export($_SESSION[$this->data['session_name']], true).';');
		$this->data[$this->data['layout_v2_name'].'_editmode'] = true;
	...
	//	$allfiles = $this->_getFiles(Yii::getPathOfAlias('application').'/views/'.$layout_folder);
	//	if($allfiles){
	//		foreach($allfiles as $k => $v){
	//			$tmp = str_replace(Yii::getPathOfAlias('application').'/views/'.$layout_folder.'/', '', $v);
	...
	if($this->data[$this->data['layout_v2_name'].'_editmode'] and isset($_SESSION[$this->data['session_name'].'_only_tree']) and $_SESSION[$this->data['session_name'].'_only_tree'] == '1'){
		...
		$layout_v2_data = Yii::getPathOfAlias('application').'/controllers/'.$layout_folder.'/data_'.$layout_v2_name_lay.'.php';

		if(file_exists(Yii::getPathOfAlias('application.controllers.'.$layout_folder).'/data_'.$layout_v2_name_lay.'.php')){
			$layout_v2_data = Yii::getPathOfAlias('application').'/controllers/'.$layout_folder.'/data_'.$layout_v2_name_lay.'.php';
		} else {
	...
	if(!empty($_POST) and isset($this->data[$this->data['layout_v2_name'].'_editmode']) and $this->data[$this->data['layout_v2_name'].'_editmode'] == true){
		 ...
		 *  $post['other2'],
		 */
		if(file_exists(Yii::getPathOfAlias('application.views.'.$layout_folder).'/'.$post['class'].'.php')){
			$file = Yii::getPathOfAlias('application.views.'.$layout_folder).'/'.$post['class'].'.php';
		} else {
	...
	if(file_exists(Yii::getPathOfAlias('application.views.'.$layout_folder).'/main.php')){
		$layout_v2_main = Yii::getPathOfAlias('application').'/views/'.$layout_folder.'/main.php';
	} else {
