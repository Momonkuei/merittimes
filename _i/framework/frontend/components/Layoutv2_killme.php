...
{
	public $layout_folder=(isset($GLOBALS['layout_folder']))?$GLOBALS['layout_folder']:'layoutv2';
...
public  function section_recursive($key)
{
	...
	if(file_exists(Yii::getPathOfAlias('application.views.'.$this->layout_folder).'/'.$type.'.php')){
		$file = Yii::getPathOfAlias('application.views.'.$this->layout_folder).'/'.$type.'.php';
	} else {
	...
	if(preg_match('/^group___(.*)___(.*)$/', $type, $matches) or ($type == 'group' and isset($this->data['layoutv2_param'][$key][1]) and $this->data['layoutv2_param'][$key][1] != '' and isset($this->data['layoutv2_param'][$key][2]) and $this->data['layoutv2_param'][$key][2] != '')){
		...
		$file = Yii::getPathOfAlias('application').'/controllers/'.$this->layout_folder.'/'.'layout_'.str_replace('/', '-',$matches[1]).'.php';
		if(file_exists($file)){
	...
	// 己支援手動載入
		...
		if(file_exists($file)){
			...
			$file2 = Yii::getPathOfAlias('application').'/controllers/'.$this->layout_folder.'/'.'datalayout_'.str_replace('/', '-',$matches[1]).'.php';
			if(file_exists($file2)){
				include $file2;
			}

			// 5. 小干涉
			$file2 = Yii::getPathOfAlias('application').'/controllers/'.$this->layout_folder.'/'.'datalayout_'.str_replace('/', '-',$matches[1]).'_'.$this->data['layout_v2_name'].'.php';
			if(file_exists($file2)){
				include $file2;
			}
...
XXX;
	...
	//} elseif(preg_match('/^view_(.*)$/', $type, $matches) and file_exists(Yii::getPathOfAlias('application').'/views/layoutv2/'.$type.'.php')){
		...
		if(file_exists(Yii::getPathOfAlias('application.views.'.$this->layout_folder).'/'.$type.'.php')){
			$return = $this->renderPartial('application.views.'.$this->layout_folder.'.'.$type, $this->data, true);
		} else {
...
public function actionDemopreview($type_backend)
{
	...
	//}
	if(file_exists(Yii::getPathOfAlias('application.views.'.$this->layout_folder).'/main.php')){
		include Yii::getPathOfAlias('application').'/views/'.$this->layout_folder.'/main.php';
	} else {
