...
<?php if(isset($this->data['layoutv2_sections_select'])):?>
	<?php if($this->data['layoutv2_sections_select'] == '1'):?><?php // 左側選單?>
		...
		<div class="menu_lg_view">
			<div>
				<p class="title-c text-left deco_1"><?php echo G::t(null,'商品分類')?></p>
			</div>
			<ul class="list-unstyled">
				<?php if(isset($this->data['layoutv2'][$this->data['section']['key']])):?>
					<?php foreach($this->data['layoutv2'][$this->data['section']['key']] as $k => $v):?>
						<li><a href="<?php if(isset($v['url']) and $v['url'] != ''):?><?php echo $v['url']?><?php else:?>#_<?php endif?>" data-toggle="collapse" data-target=".submenu_<?php echo $k?>"><?php echo $v['name']?></a>
...
<?php endif?>
	...
	<?php elseif($this->data['layoutv2_sections_select'] == '2'):?><?php // 縮小後才會出現的左側選單，記得外層是Bbox_1c hidden-md hidden-lg?>
		...
		<div data-toggle="collapse" data-target=".menu_s_view" aria-expanded="fale">
			<p class="title-sm text-center margin_sm_tb" style="cursor:pointer"><?php echo G::t(null,'商品分類')?><span class="caret hidden-md hidden-lg"></span></p>
		</div>
			...
			<ul class="list-unstyled">
				<?php if(isset($this->data['layoutv2'][$this->data['section']['key']])):?>
					<?php foreach($this->data['layoutv2'][$this->data['section']['key']] as $k => $v):?>
						<li><a href="<?php if(isset($v['url']) and $v['url'] != ''):?><?php echo $v['url']?><?php else:?>#_<?php endif?>" data-toggle="collapse" data-target=".submenu_<?php echo $k?>"><?php echo $v['name']?></a>
							<?php if(isset($v['childs']) and count($v['childs']) > 0):?>
								<ul class="submenu_<?php echo $k?> nav nav-pills nav-stacked <?php if(isset($_GET['id']) and $producttype_tmp[$_GET['id']]['pid'] == $v['id']):?><?php else:?> collapse <?php endif?> ">
								<?php foreach($v['childs'] as $kk => $vv):?>
									<li><a href="<?php echo $vv['url']?>"><?php echo $vv['name']?></a></li>
								<?php endforeach?>
								</ul>
	...
	<?php elseif($this->data['layoutv2_sections_select'] == '6'):?><?php // 商品內頁編輯器內容 ?>
		...
		<div class="box_ad_1c">
			<div>
				<div class="title-sm"><?php echo G::t(null,'Product Features')?></div>
				<div id="pro_features">
					<?php echo $tmp['describe2']?>
				</div>
				<div class="title-sm"><?php echo G::t(null,'Notice')?></div>
				<div id="pro_notice">
					<p align="left">
		...
		<div class="text-center">
			<p class="title"></p>
			<a href="javascript:history.back();" class="btn btn-default"><?php echo G::t(null,'BACK')?></a>
		</div>
			...
			<div class="checkitem col-sm-12 hidden-xs hidden-sm">
				<div>
					<div class="Bbox_in_2c_L3 col-sm-10 col-md-11 col-md-offset-1">
						<div>
						<?php if(count($this->data['layoutv2'][$this->data['section']['key']])):?>
							<div>
								<div>
...
XXX;
?>
<?php elseif($this->data['layoutv2_sections_select'] == '9'):?><?php // 商品內頁大圖特效 + 燈箱 ?>
		[POS1]
