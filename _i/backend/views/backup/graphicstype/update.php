...
<?php $this->data['other_content_title'] = $updatecontent['name_parent'].'ggg'?>
...
<div class="row">
	<div class="col-md-12">
		...
		<div class="color-panel hidden-phone">
			...
			<form onsubmit="searchkeyword(this);return false;" name="searchFrm" id="searchFrm" action="http://rwd.devel2.buyersline.com.tw/_i/backend.php?r=item1type2/update" method="GET" style="position: absolute;right:0px;top:4px;">


				<div class="searchCopy"> <span class="fle">搜尋</span>
					<input type="text" placeholder="搜尋資料內容" id="search_keyword" name="search_keyword">
					<input type="image" align="top" src="http://889.devel.gisanfu.idv.tw/_i/framework/backend/assetsg/template/admin_yiiv_5/images/icon007.gif" title="搜尋" class="search_icon">
									</div>
		...
		<ul class="page-breadcrumb breadcrumb">
			<li>
				<i class="icon-home"></i>
				<?php if(isset($this->data['breadcrumbs']) and count($this->data['breadcrumbs']) > 0):?>
					<?php foreach($this->data['breadcrumbs'] as $k => $v):?>
						<a href="<?php echo $this->createUrl($this->data['router_class'].'/update',array('param'=>'v'.$k))?>"><?php echo $v?></a> 
	...
	<div class="portlet-body">
		...
		<?php if(!empty($def['updatefield'])):?>
			...
			<div class="buttons indexgo03" style2="clear:both;">
				<button class="btn blue" type="submit"><?php G::te($this->data['theme_lang'], 'Submit', null, '送出')?></button>
...
<?php if(0):?>
				...
				<button onclick="document.location.href='<?php echo $class_url?>';" class="btn green" type="button"><?php G::te($this->data['theme_lang'], 'Previous', null, '上一頁')?></button>
				<?php else:?>
				<button onclick="document.location.href='<?php echo $this->createUrl($this->data['router_class'].'/update',array('param'=>'v'.$this->data['params']['value'][0]))//$prev_url?>';" class="btn green" type="button"><?php G::te($this->data['theme_lang'], 'Previous', null, '上一頁')?></button>
				<?php endif?>
				<button onclick="document.location.href='<?php echo $this->createUrl($this->data['router_class'].'/update',array('param'=>'v'.$this->data['params']['value'][0]))//$prev_url?>';" class="btn green" type="button"><?php G::te($this->data['theme_lang'], 'Previous', null, '上一頁')?></button>
<?php endif?>
<?php //var_dump($this->data['params'])?>
			</div>
			...
			<?php if(isset($this->data['params']['value'][0]) && $this->data['params']['value'][0]!=''):?>
			<input type="hidden" name="prev_url" value="<?php echo $this->createUrl($this->data['router_class'].'/update',array('param'=>'v'.$this->data['params']['value'][0]))//$prev_url?>" />
			<?php else:?>
		...
		<?php if(isset($updatecontent['pid']) and $updatecontent['allow_child'] === true): //and $updatecontent['pid'] == 0):?>
