...
class NonameController extends Controller {
	...
	protected function beforeAction($action)
	{
		...
		// 	'type' => 'input',
		// 	'attr' => array(
		// 		'id' => 'sys_config_欄位英文名稱_'.$this->data['admin_switch_data_ml_key'],
		// 		'name' => 'sys_config_欄位英文名稱_'.$this->data['admin_switch_data_ml_key'],
		// 	),
		// );
		// $this->def['updatefield']['sections'][0]['field']['sys_config_欄位英文名稱_'.$this->data['admin_switch_data_ml_key']] = $tmp;

		// 圖片欄位範例，一個網站可以有十張單頁的圖片 (1/2)
		// $tmp = array(
		// 	'label' => '圖片',
		// 	'type' => 'fileuploader',
		// 	'other' => array(
		...
		// 		'width' => '1000',
		// 		'height' => '652',
		// 		'comment_size' => '500x758',
		// 		'no_ext' => '',
		// 		'no_need_delete_button' => '',
		// 		'html_end' => '', // 這裡會跟進scss產品列表圖比例(proImgSizeType)而變化
		// 	),
		// );
		// $this->def['updatefield']['sections'][0]['field']['pic5_'.$this->data['admin_switch_data_ml_key']] = $tmp;
	...
	public function actionIndex($param='')
	{
		if(empty($_POST)){
			...
			$list = array(
				...
				// 非圖片欄位範例
				// '欄位英文名稱_'.$this->data['admin_switch_data_ml_key'],

				// 圖片欄位範例，一個網站可以有十張單頁的圖片 (2/2)
				// 'pic5_'.$this->data['admin_switch_data_ml_key'],
			);
			...
			// 2018-07-26 新增圖片功能，整個網站的單頁，共可以使用１０張
			for($x=1;$x<=10;$x++){
				$k = 'pic'.$x.'_'.$this->data['admin_switch_data_ml_key'];
				if(isset($sys_configs[$k])){
					$this->data['updatecontent']['pic'.$x.'_'.$this->data['admin_switch_data_ml_key']] = $sys_configs[$k];
				}
			}
			...
			// 2018-07-26 新增圖片功能，整個網站的單頁，共可以使用１０張
			for($x=1;$x<=10;$x++){
				$k1 = 'pic'.$x.'_'.$this->data['admin_switch_data_ml_key'];
				$k2 = 'pic'.$x;
				if(isset($save[$k1])){
					$v1 = $save[$k1];
			...
			if(count($save) > 0){
				...
				foreach($save as $k => $v){
					...
					if(isset($this->data['sys_configs'][$k]) and $v != $this->data['sys_configs'][$k]){
						...
						$this->cidb->where('keyname', $k);
						$this->cidb->update('sys_config', array('keyval'=>$v)); 
						...
						$this->cidb->insert('sys_config', array('keyname'=>$k,'keyval'=>$v)); 
