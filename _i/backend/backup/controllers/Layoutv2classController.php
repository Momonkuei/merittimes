...
class Layoutv2classController extends Controller {
	...
	protected $def = array(
		...
		'sortable' => array(
			...
			'url' => 'backend.php?r=layoutv2class/sort', // ajax post都會有個目標
		),
		// 建立前端要顯示的欄位
	...
	protected function beforeAction($action)
	{
		...
		$rows = $this->db->createCommand()->from('layoutv2')->where('is_enable=1 and type=:type and ml_key=:ml_key', array(':type'=>'layoutv2classtype',':ml_key'=>$this->data['admin_switch_data_ml_key']))->order('sort_id asc')->queryAll();
		...
		$condition = ' type=\'layoutv2class\' and ml_key=\''.$this->data['admin_switch_data_ml_key'].'\' ';
		$condition_sortable = ' type="layoutv2class" and ml_key="'.$this->data['admin_switch_data_ml_key'].'" ';
	...
	{
		$this->data['def']['updatefield']['sections'][1]['field']['iframe01']['attr']['src'] = '/index.php?r=site/demopreview&type_backend='.$this->data['updatecontent']['other1'];
		$this->data['def']['updatefield']['sections'][1]['field']['iframe01']['attr']['width'] = '100%';
		$this->data['def']['updatefield']['sections'][1]['field']['iframe01']['attr']['height'] = '500';
	}
