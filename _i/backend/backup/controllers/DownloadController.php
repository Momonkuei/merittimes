...
class NonameController extends Controller {
	...
	protected function index_last($param='')
	{
		...
		if($this->data['listcontent']){
			foreach($this->data['listcontent'] as $k => $v){
				...
				// 2017-10-11 下載次數，從agent_log中找出來的
				$rows = $this->cidb->select('id')->where('item_id', $v['id'])->get('agent_log')->result_array();
				$v['_count'] = count($rows);			
	...
	{
		//$this->data['def']['updatefield']['sections'][1]['field']['kc01']['other']['school_id'] = 'download'.$this->data['updatecontent']['id'];
	}
