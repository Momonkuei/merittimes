...
class NonameController extends Controller {
			...
			'sections' => array(
				...
				array(
					...
					'field' => array(
						...
						'applications_num' => array(
							'label' => '計畫申請次數',
							'type' => 'label',
							'attr' => array(
								'id' => 'applications_num',
								'name' => 'applications_num',
								'size' => '40',
							),
						),
						// 'file1' => array(
	...
	protected function update_show_last($updatecontent)
	{
		...
		$updatecontent['file1'] = 'assets/file/'.$this->data['router_class'].'/'.$updatecontent['file1'];
		if(!empty($this->data['updatecontent']['addr']) && stristr($this->data['updatecontent']['addr'],'_')){
			// print_r($this->data['updatecontent']['addr']);die;
			$this->data['updatecontent']['addr']=explode('_',$this->data['updatecontent']['addr']);
			$address='';
			foreach($this->data['updatecontent']['addr'] as $k => $v){
				if($k==0 && !empty($v) && $v!='no'){
					$address.=$v;
				}else if($k==1 && !empty($v) && $v!='no'){
					$address.=$v;
				}else if($k==2 && !empty($v) && $v!='no'){
					$address.=$v.'路/街';
				}else if($k==3 && !empty($v) && $v!='no'){
					$address.=$v.'段';
				}else if($k==4 && !empty($v) && $v!='no'){
					$address.=$v.'巷';
				}else if($k==5 && !empty($v) && $v!='no'){
					$address.=$v.'弄';
				}else if($k==6 && !empty($v) && $v!='no'){
					$address.=$v.'號';
				}else if($k==7 && !empty($v) && $v!='no'){
					$address.='之'.$v.'．';
				}else if($k==8 && !empty($v) && $v!='no'){
					$address.=$v.'樓';
				}else if($k==9 && !empty($v) && $v!='no'){
					$address.='之'.$v;
				}else if($k==10 && !empty($v) && $v!='no'){
					$address.='．'.$v;
				}
			}
			// echo $address;die;
			$this->data['updatecontent']['addr']=$address;
		}
		if($this->data['updatecontent']['other3']==1){
			$this->data['updatecontent']['other3']='國小組';
		}else if($this->data['updatecontent']['other3']==2){
			$this->data['updatecontent']['other3']='國中組';
		}else if($this->data['updatecontent']['other3']==3){
			$this->data['updatecontent']['other3']='高中職組';
		}else if($this->data['updatecontent']['other3']==4){
			$this->data['updatecontent']['other3']='大專校院組';
		}
	...
	public function actionMemberlogin()
	{
		...
		$id = $_GET['param']; // 場次ID
		//場次資料
		$url=str_replace('_i','',$this->data['base_url']).'apply_post.php?bkmemberlogin=1&member_d='.$id;
		// $listcontent = $this->db->createCommand()->from('pray')->where('id=:id',array(':id'=>$id))->queryRow();
	...
	public function actionMemberlist()
	{
		...
		$id = $_GET['param']; // ID
		$url=$this->data['base_url'].'/backend.php?r=memberlist&d='.$id;
		header("Location: $url", TRUE, 301);
	}
	//班級列表頁
	public function actionClasslist()
	{
		...
		$id = $_GET['param']; // ID
		$url=$this->data['base_url'].'/backend.php?r=writeplan_class&d='.$id;
		header("Location: $url", TRUE, 301);
	}
	...
	public function actionExcelexport2()
	{
		...
		if($admin_field and !empty($admin_field)){
			...
			//  <br /><a href="#funcfieldv3_sms_text">(#)</a>
		}
		// print_r($admin_field);die;
		...
		if($rows and !empty($rows)){
			foreach($rows as $k => $v){
				if(!empty($v['addr']) && stristr($v['addr'],'_')){
					$v['addr']=explode('_',$v['addr']);
					$address='';
					foreach($v['addr'] as $kkk => $vvv){
						if($kkk==0 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv;
						}else if($kkk==1 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv;
						}else if($kkk==2 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv.'路/街';
						}else if($kkk==3 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv.'段';
						}else if($kkk==4 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv.'巷';
						}else if($kkk==5 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv.'弄';
						}else if($kkk==6 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv.'號';
						}else if($kkk==7 && !empty($vvv) && $vvv!='no'){
							$address.='之'.$vvv.'．';
						}else if($kkk==8 && !empty($vvv) && $vvv!='no'){
							$address.=$vvv.'樓';
						}else if($kkk==9 && !empty($vvv) && $vvv!='no'){
							$address.='之'.$vvv;
						}else if($kkk==10 && !empty($vvv) && $vvv!='no'){
							$address.='．'.$vvv;
						}
					}
					$rows[$k]['addr']=$address;
