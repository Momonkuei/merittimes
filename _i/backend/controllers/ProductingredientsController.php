	...
	protected function beforeAction($action)
	{
		...
		if($rowg and isset($rowg['other25']) and $rowg['other25'] != ''){
			if(isset($session_node['value']) and $session_node['value'] > 0){
				...
				// 找上一層的標題名稱
				if($is_html){
					$row = $this->cidb->select('*, topic as name')->where('type',"productingredientsgroup")->where('id',$session_node['value'])->get('html')->row_array();
				} else {
				...
				// 	print_r($this->data['admin_menu_breadcrumbs']);
					// $this->def['title'] = $row['topic'];
					$mainpage="";
					if(!empty($_SESSION["mainpage"])){
					    $mainpage=$_SESSION["mainpage"];
					}
					$this->data['main_content_title'] = $mainpage.'<a href="backend.php?r=productingredientsgroup/index">'.$row['name'].'</a> / '.$rowg['topic'];
				}
			} else {
