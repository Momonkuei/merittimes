	...
	protected function beforeAction($action)
	{
		...
		if($rowg and isset($rowg['other25']) and $rowg['other25'] != ''){
			if(isset($session_node['value']) and $session_node['value'] > 0){
				...
				// 找上一層的標題名稱
				if($is_html){
					$row = $this->cidb->select('*, topic as name')->where('type',$session_node['parent'])->where('id',$session_node['value'])->get('html')->row_array();
				} else {
					$row = $this->cidb->where('id', $session_node['value'])->get($session_node['parent'])->row_array();
				}
				...
				if($row and isset($row['id'])){
					...
					$_SESSION["mainpage"]='<a href="backend.php?r='.$session_node['parent'].'">'.$rowg_parent['topic'].'</a> / <a href="backend.php?r='.$session_node['parent'].'/update&param=v'.$session_node['value'].'">'.$row['name'].'</a> / ';
				}
			} else {
...
';
	...
	public function actionproductIngredients($param)
     {
        $ss = 'productingredients_node';
        $session = Yii::app()->session[$ss];
        ...
        $this->redirect($this->createUrl('productingredients/index'));
    }
