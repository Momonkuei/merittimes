...
class NonameController extends Controller {
	...
	protected $def = array(
		...
		'index_buttons' => array(
			array(
				'name' => '匯出(資訊用)ps.僅核可資料可匯出<i class="icon-external-link"></i>',  // 按鈕的名稱和圖示
				...
				'onclick' => 'javascript:location.href=\'backend.php?r=writeplan/excelexport1\'',
			),
			array(
				'name' => '匯出(一般)<i class="icon-external-link"></i>',  // 按鈕的名稱和圖示
				'name2' => 'export', // 假設create，那權限也是用create，那該功能也要開create(admin_resource)，雖然create早就有了，這裡只是範例而以
				'id' => '', // button
				'class' => 'btn btn-info', // button
				// 'onclick' => 'javascript:location.href=\'XXX\'',
				'onclick' => 'javascript:location.href=\'backend.php?r=writeplan/excelexport2\'',
			),
		),
		'table' => 'writeplan',
		'orm' => 'Empty_orm',
...
$('#end_date').datepicker({dateFormat: 'yy-mm-dd'});
	...
	protected function beforeAction($action)
	{
		...
		if($rowg){ 
			if(isset($rowg['pic3']) and $rowg['pic3'] == 1){ // 有日期排序的情況下
				...
				if(isset($rowg['pic2']) and $rowg['pic2'] == 0 && $this->data['router_class']!='writeplan'){ // 資料流沒分類 全部都用 sort_id
        ...
        $rows = $this->db->createCommand()->from('member')->where('is_enable=1')->order('id asc')->queryAll();
		if($rows and !empty($rows)){
            $this->def['searchfield']['sections'][0]['field']['modifier']['other']['values'][0] = '請選擇';
			foreach($rows as $k => $v){
				$this->def['searchfield']['sections'][0]['field']['modifier']['other']['values'][$v['name']] = $v['name'];
			}
		}
        //學期資料    
		$semester=$this->cidb->select('id,topic')->where('type','semester')->where('is_enable',1)->where('ml_key',$this->data['ml_key'])->order_by('sort_id')->get('html')->result_array();
		$semester_array=array();
		foreach($semester as $k => $v){
			$this->def['searchfield']['sections'][0]['field']['semester']['other']['values'][$v['id']] = $v['topic'];
		}
	...
	public function actionPreview()
	{
		...
		$id = $_GET['param']; // ID
		$rowg = $this->cidb->where('id',$id)->where('type','news')->where('ml_key',$this->data['admin_switch_data_ml_key'])->get('html')->result_array();
		$url=str_replace('_i','',$this->data['base_url']);
		$url=$url.'newsdetail_'.$this->data['admin_switch_data_ml_key'].'.php?id='.$rowg[0]['id'];
	...
	protected function index_last($param='')
	{
		...
		//學期資料
		$semester=$this->cidb->select('id,topic')->where('type','semester')->where('is_enable',1)->where('ml_key',$this->data['ml_key'])->get('html')->result_array();
		$semester_array=array();
		foreach($semester as $k => $v){
			$semester_array[$v['id']]=$v['topic'];
		}
		if($this->data['listcontent']){
			foreach($this->data['listcontent'] as $k => $v){
				/*學期*/
				if(isset($v['semester']) && !empty($v['semester'])){
					$v['semester']=$semester_array[$v['semester']];
				}
				//編號(非流水號)
				...
				if(!empty($v['file3'])){
					$customer_data=$this->db->createCommand()->from('customer')->where('is_enable=1 and id='.$v['member_id'])->queryRow();
					$_href = '/_i/assets/file/'.$this->data['router_class'].'/'.$customer_data['code'].'/'.$v['id'].'/'.$v['file3'];	
					$v['file3'] = '<a href="'.$_href.'" target="_BREAK">核章檔案</a>';
				}
	...
	public function actionPraylist()
	{
		if(!isset($params['prev']) or $params['prev'] == ''){
			$params['prev'] = $this->data['current_base64_url'];
		}
		$id = $_GET['param']; // ID
		$url=$this->data['base_url'].'/backend.php?r=praylist&d='.$id;
		header("Location: $url", TRUE, 301);
	}
	...
	protected function update_show_last($param='')
	{
        ...
        $data_array=$this->cidb->where('class_id',$this->data['updatecontent']['id'])->order_by('create_time desc')->get('writeplanreview')->result_array();
		$this->data['writeplanreview'] = $data_array;

		if(!empty($this->data['updatecontent'])){
			...
			$this->data['updatecontent']['semester']=$semester_list[$this->data['updatecontent']['semester']];
	 ...
	 * 2020-02-10
	 */
	public function actionExcelexport1()
	{
		...
		// }
		$admin_field=array(
			'school_name'=>array('label'=>'學校全銜'),
			'other2'=>array('label'=>'學校統編'),
			'other3'=>array('label'=>'類別'),
			'other4'=>array('label'=>'行政區'),
			'addr'=>array('label'=>'詳細地址'),
			'addr1'=>array('label'=>'地址_郵遞區號'),
			'addr2'=>array('label'=>'地址_路街'),
			'addr3'=>array('label'=>'地址_段'),
			'addr4'=>array('label'=>'地址_巷'),
			'addr5'=>array('label'=>'地址_弄'),
			'addr6'=>array('label'=>'地址_號'),
			'addr7'=>array('label'=>'地址_號之'),
			'addr8'=>array('label'=>'地址_樓'),
			'addr9'=>array('label'=>'地址_樓之'),
			'addr10'=>array('label'=>'地址_其他'),
			'name'=>array('label'=>'聯絡人_姓名'),
			'phone'=>array('label'=>'聯絡人_電話'),
			'phone_area'=>array('label'=>'聯絡人_電話區碼'),
			'p_extension'=>array('label'=>'聯絡人_電話分機'),
			'mobile'=>array('label'=>'聯絡人_行動電話'),
			'email'=>array('label'=>'聯絡人_Email'),
			'fax'=>array('label'=>'聯絡人_傳真'),
			'fax_area'=>array('label'=>'聯絡人_傳真區碼'),
			'semester_name'=>array('label'=>'申請期別'),
			'review_time'=>array('label'=>'審查日期'),
			'approved_num'=>array('label'=>'核可報份'),
		);	
		//$query = 'SELECT * FROM '.$this->data['router_class'];
		$query = 'SELECT a.id,a.modifier,a.a_results,a.approved_num,a.review_time,a.semester,b.school_name,b.name,b.other2,b.other3,b.other4,b.addr,b.phone_area,b.phone,b.p_extension,b.mobile,b.email,b.fax,b.fax_area,c.topic as semester_name FROM '.$this->def['table'] .' as a left join customer as b on a.member_id=b.id left join html as c on c.type="semester" and c.id=a.semester '; //2017/5/9 by lota fix 
		// if(isset($this->def['condition'][0][1]) and trim($this->def['condition'][0][1]) != ''){
			$query .= ' WHERE a.a_results="核可"';
			$where='';
			if(!empty($_SESSION['writeplan_search'])){
				foreach($_SESSION['writeplan_search'] as $where_k => $where_v){
					if(!empty($where_v) && $where_k!='keyword' && $where_k!='a_results'){
						$where.=' and  a.'.$where_k.'=\''.$where_v.'\'';
					}else if($where_k=='keyword'){
						$where.=' and  a.name like \'%'.$where_v.'%\'';
					}
		...
		foreach($rows as $k => $v){
			if(stristr($v['addr'],'_')==true){
				$addr_array=explode('_',$v['addr']);
				if(is_array($addr_array)){
					foreach($admin_field as $kk => $vv){
						$value='';
						switch($kk){
							case 'addr':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==0 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v;
									}else if($addr_k==1 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v;
									}else if($addr_k==2 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'路/街';
									}else if($addr_k==3 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'段';
									}else if($addr_k==4 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'巷';
									}else if($addr_k==5 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'弄';
									}else if($addr_k==6 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'號';
									}else if($addr_k==7 && !empty($addr_v) && $addr_v!='no'){
										$value.='之'.$addr_v.'．';
									}else if($addr_k==8 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'樓';
									}else if($addr_k==9 && !empty($addr_v) && $addr_v!='no'){
										$value.='之'.$addr_v;
									}else if($addr_k==10 && !empty($addr_v) && $addr_v!='no'){
										$value.='．'.$addr_v;
									}
								}
								$rows[$k]['addr']=$value;
							...
							case 'addr2':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==2 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'路/街';
									}
								}
								$rows[$k]['addr2']=$value;
							...
							case 'addr3':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==3 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'段';
									}
								}
								$rows[$k]['addr3']=$value;
							...
							case 'addr4':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==4 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'巷';
									}
								}
								$rows[$k]['addr4']=$value;
							...
							case 'addr5':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==5 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'弄';
									}
								}
								$rows[$k]['addr5']=$value;
							...
							case 'addr6':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==6 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'號';
									}
								}
								$rows[$k]['addr6']=$value;
							...
							case 'addr7':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==7 && !empty($addr_v) && $addr_v!='no'){
										$value.='之'.$addr_v.'．';
									}
								}
								$rows[$k]['addr7']=$value;
							...
							case 'addr8':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==8 && !empty($addr_v) && $addr_v!='no'){
										$value.=$addr_v.'樓';
									}
								}
								$rows[$k]['addr8']=$value;
							...
							case 'addr9':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==9 && !empty($addr_v) && $addr_v!='no'){
										$value.='之'.$addr_v;
									}
								}
								$rows[$k]['addr9']=$value;
							...
							case 'addr10':
								foreach($addr_array as $addr_k => $addr_v){
									if($addr_k==10 && !empty($addr_v) && $addr_v!='no'){
										$value.='之'.$addr_v;
									}
								}
								$rows[$k]['addr10']=$value;
	...
	public function actionExcelexport2()
	{
		...
		$admin_field=array(
			'name'=>array('label'=>'標題'),
			'other4'=>array('label'=>'學期'),
			'president_name'=>array('label'=>'校長姓名'),
			'landline'=>array('label'=>'連絡電話'),
			'extension'=>array('label'=>'分機'),
			'phone'=>array('label'=>'手機'),
			'email'=>array('label'=>'信箱'),
			'description'=>array('label'=>'簡述'),
			'implement_date'=>array('label'=>'實施日期'),
			'course'=>array('label'=>'方式與經歷'),
			'remark'=>array('label'=>'備註'),
			'class_name'=>array('label'=>'班級數量'),
			'student_name'=>array('label'=>'學生數量'),
			'actual_num'=>array('label'=>'預設報份'),
			'apply_num'=>array('label'=>'希望報份'),
			'approved_num'=>array('label'=>'核可報份'),
			'a_results'=>array('label'=>'審核結果'),
			'r_comments'=>array('label'=>'回覆意見'),
			'file1'=>array('label'=>'檔案1'),
			'file2'=>array('label'=>'檔案2'),
			'file3'=>array('label'=>'核章檔案'),
		);	

		//$query = 'SELECT * FROM '.$this->data['router_class'];
		$query = 'SELECT * FROM '.$this->def['table']; //2017/5/9 by lota fix 
		if(isset($this->def['condition'][0][1]) and trim($this->def['condition'][0][1]) != ''){
			$where=str_replace('\\','',$this->def['condition'][0][1]);
			$query .= ' WHERE '.$where;
		}
		$rows = $this->db->createCommand($query)->queryAll();
