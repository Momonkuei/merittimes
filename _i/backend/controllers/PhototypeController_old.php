...
class NonameController extends Controller {
	...
	protected $def = array(
		...
		'updatefield' => array(
			...
			'sections' => array(
				...
				// 這是SEO欄位的範本，如果你需要，就打開它 1/4 lota補充：這個要記得放在array[2]，才會自動產生
				array(
					'form' => array('enable' => false),
					'type' => '1',
					'field' => array(
	...
	protected function beforeAction($action)
	{
		...
		//判別有無分類描述功能
		unset($_constant);
		eval('$_constant = '.strtoupper(str_replace('type','',$this->data['router_class'])).'_TYPE_DETAIL;');
		if(!$_constant){
			unset($this->def['updatefield']['sections'][1]);
		}else{
			//判斷SEO權限
			$acl = new Admin_acl();
			$acl->start();
		...
		//判別產品預設顯示方式
		unset($_constant);
		eval('$_constant = '.strtoupper(str_replace('type','',$this->data['router_class'])).'_DETAIL_SHOW_TYPE;');
		if($_constant==1){
			unset($this->def['updatefield']['sections'][0]['field']['pic1']);
		...
		if(file_exists(Yii::getPathOfAlias('system.backend.controllers.seo').'.php') && $_constant ){
			$seo_func = 'a';
	...
	//刪除前判斷該分類是否有產品，如有則提醒 不執行刪除
	protected function delete_before($array) 
	{
		
		//var_dump($array);die;
		//檢查該分類有無子分類
		$rows1 = $this->db->createCommand()->from($this->data['def']['table'])->where('is_enable=1 and ml_key="'.$this->data['admin_switch_data_ml_key'].'" AND pid='.$array['id'])->queryAll();
		if($rows1 && count($rows1)){			
			L::alert_and_back(G::t(null,'該分類下有子分類！請先將子分類刪除後再進行動作'), $this->data);
		}

		//檢查該分類有無產品
		$rows = $this->db->createCommand()->from('html')->where('is_enable=1 and type="'.str_replace('type','',$this->data['router_class']).'" and ml_key="'.$this->data['admin_switch_data_ml_key'].'" AND class_id='.$array['id'])->queryAll();
		if($rows && count($rows)){			
			L::alert_and_back(G::t(null,'該分類下有產品！請先將產品刪除後再進行動作'), $this->data);
		}
	}
