...
class NonameController extends Controller {
	...
	{		
		...
		if(empty($_GET['d']) && !isset($_SESSION['writeplan_id'])){
			$url='backend.php?r=writeplan';
			G::alert_and_redirect('請先選擇計畫!', $url, $this->data);
		}
		// 懶得改Controller的名稱之二
		// 為了簡化複製後，所要做的動作
		$this->def['table'] = 'writeplan_file';
		$this->def['empty_orm_data']['table'] = 'writeplan_file';
		...
		if(!empty($_GET['d'])){
			$_SESSION['writeplan_id']=$_GET['d'];
			$class_toid=$_SESSION['writeplan_id'];
		}else if(!empty($_SESSION['writeplan_id'])){
			$class_toid=$_SESSION['writeplan_id'];
		}
		...
		$condition = ' 1 ';
		if(!empty($class_toid)){
			$condition .=' and writeplan_id=\''.$class_toid.'\'';
		}
    ...
    public function actionFileopen()
	{
		...
		$id = $_GET['param']; // ID
        $file_data=$this->cidb->where('id',$id)->get('writeplan_file')->row_array();
        $school_code=$this->cidb->where('id',$file_data['member_id'])->get('customer')->row_array();
        if(!empty($file_data['class_id'])){
            $clss_code=$this->cidb->where('id',$file_data['class_id'])->get('writeplan_class')->row_array();
        }
		$url=$this->data['base_url'].'/assets/file/writeplan/'.(!empty($school_code)?$school_code['code'].'/':'all_school/').(isset($file_data['writeplan_id'])?$file_data['writeplan_id'].'/':'').$file_data['file1'];
