...
$(\"body\").on(\"blur\",\".edit_name\",function(e){
	var txt = $(this).val();
	var id = $(this).data(\"id\");
	$.get(\"/_i/backend.php?r=productphoto/edit_name\", { name: txt,id:id },function( data ){
		//location.reload();
		} );
});
",
				...
				array(
					'form' => array(
						...
						'attr' => array(
							...
							'action' => '/_i/backend.php?r=productphoto/fileupload',
						),
					),
					'type' => '1',
					'field' => array(
				...
				array(
					'form' => array(
						...
						'attr' => array(
							...
							'name' => 'form_data_search',
							'method' => 'post',
							'action' => '/_i/backend.php?r=productphoto/search',
						),
					),
					'type' => '1',
					'field' => array(
					...
					<button class="btn red" type="submit" onclick="javascript:location.href=\'/_i/backend.php?r=productphoto/cancel_search\';return false;"><i class="icon-remove"></i>取消搜尋</button>',
							),
						),
...
XXX;
		...
		if( !empty($_FILES) ){
			...
			foreach ($_FILES as $key => $value) {
				if( !empty($value['name']) && $value['error']=="0"  ){//檢查檔案 START
					...
					//UPDATE `productphoto` set sort_id = sort_id +1 where class_id =66
	...
	public function actionEdit_name()
    {
    	...
    	$this->cidb->update('productphoto',array('name'=>$name));
