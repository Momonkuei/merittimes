...
class NonameController extends Controller {
			...
			'sections' => array(
				array(
					...
					/*
						...
						'sys_config_watermark_percent' => array(
							'label' => '浮水印縮圖百分比',
							'type' => 'select3',
							//'type' => 'select5',
							//'merge' => '1', // 頭中尾的頭(1)
							'attr' => array(
								'id' => 'sys_config_watermark_percent',
								'name' => 'sys_config_watermark_percent',
							),
							'other' => array(
								'values' => array(
									'10' => '10%',
								...
								//),
								'default' => '50',
								'html_end' => '* 如果浮水印的圖，比原圖大的情況下，這個設定才有作用',
							),
						),
					),
				),
				array(
					...
					'field' => array(
						...
						'iframe01_comment' => array(
							'label' => '浮水印效果展示：',
							'type' => 'inputn',
							'other' => array(
								'html' => " (實際狀況要依照圖片和浮水印的大小)",
							),
						),
						// 浮水印展示
						'iframe01' => array(
							...
							'attr' => array(
								...
								'width' => '100%',
								'height' => '500px',
								'src' => '/_i/assets/upload/_demo/watermarkdemo.jpg',
								'html_end' => '123',
							),
						),
						// 無損壓縮浮水印檔案
						...
						//		'width' => '100%',
						//		'height' => '0px',
						//		'src' => '/_i/image_optimizer.php?src=all.png',
						//	),
						//),
					),
				),
	...
	public function actionIndex($param='')
	{
		if(empty($_POST)){
			...
			// 2017-12-25
			// 功能是否有作用，這裡有功能，但是只是寫虛擬碼而以
			// $watermark_status = false;
			// if(file_exists(tmp_path.'/../assets/.htaccess')){
			// 	$htaccess_tmp = file_get_contents(tmp_path.'/../assets/.htaccess');
			// 	$htaccess_tmps = explode("\n", $htaccess_tmp);
			// 	foreach($htaccess_tmps as $k => $v){
			// 		if(preg_match('/^#/', $v)){
			// 			// do nothing
			// 		} else {
			// 			if(preg_match('/timthumb/', $v) and !preg_match('/nowatermark/', $v)){
			// 				echo $v;
			...
			// 清除圖片快取			
			$log = str_replace('assets','cache2',tmp_path);
			$this::SureRemoveDir($log , false); // 第二個參數: true 連 chech2 目錄也刪除
			...
			$tmp = array(
				...
				'percent' => $save['sys_config_watermark_percent'],
			);
