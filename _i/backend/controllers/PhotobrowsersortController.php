...
class NonameController extends Controller {
	...
	protected $def = array(
		...
		'default_sort_field' => 'sort_id_browser', // 預設要排序的欄位 2016/04/29 改為預設為id 如有搜尋分類,則動態改為sort_id 2016/6/15 因點選分類後會無法排序，再度改回sort_id
		'func_field' => array(
			'id' => 'id',
			'sort_id' => 'sort_id_browser',
		),
	...
	protected function beforeAction($action)
	{
		...
		$product_table = 'html';
		$class_name = str_replace('browsersort', '', $this->data['router_class']); // 為了支援產品的首頁排序
		...
		$this->def['condition'][0][1] = 'is_enable=1 and type=\''.$class_name.'\' and ml_key=\''.$this->data['admin_switch_data_ml_key'].'\' ';
		$this->def['sortable']['condition'] = 'is_enable=1 and type="'.$class_name.'" and ml_key="'.$this->data['admin_switch_data_ml_key'].'"';
	...
	protected function index_last($param='')
	{
		...
		if($this->data['listcontent']){
			foreach($this->data['listcontent'] as $k => $v){
				if($v['pic1'] != ''){
					$v['pic1'] = $this->data['image_upload_path'].'/'.str_replace('browsersort', '', $this->data['router_class']).'/'.$v['pic1'];
				}
