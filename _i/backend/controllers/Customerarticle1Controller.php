...
$contentx = file_get_contents(Yii::getPathOfAlias('system').ds('/').target_app_name.ds('/controllers/').'shoparticle2Controller.php');
$contentx = str_replace('<'.'?'.'php', '', $contentx);
$contentx = str_replace('shoparticle2', 'Articlexxxsingle', $contentx); // class extents
$contentx = str_replace('shoparticle2', 'company', $contentx); // 欄位
eval($contentx);
...
class NonameController extends Controller {
	...
	protected function beforeAction($action)
	{
		...
		$tmp = $this->def['updatefield']['sections'][0]['field']['sys_config_'];
		$tmp['attr']['id']   .= $this->data['router_class'].'_'.$this->data['admin_switch_data_ml_key'];
		$tmp['attr']['name'] .= $this->data['router_class'].'_'.$this->data['admin_switch_data_ml_key'];
		$this->def['updatefield']['sections'][0]['field']['sys_config_'.$this->data['router_class'].'_'.$this->data['admin_switch_data_ml_key']] = $tmp;

		unset($this->def['updatefield']['sections'][0]['field']['sys_config_']);
