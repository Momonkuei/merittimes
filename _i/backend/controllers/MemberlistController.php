...
class NonameController extends Controller {
	...
	{		
		...
		// 懶得改Controller的名稱之二
		// 為了簡化複製後，所要做的動作
		$this->def['table'] = 'customer';
		$this->def['empty_orm_data']['table'] = 'customer';
		...
		if(!empty($_GET['d'])){
			$_SESSION['member_class']=$_GET['d'];
			$class_toid=$_SESSION['member_class'];
		...
		$condition = ' 1 ';
		if(!empty($class_toid)){
			$condition .=' and class_id=\''.$class_toid.'\'';
		}
