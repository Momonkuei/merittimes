...
class NonameController extends Controller {
	...
	protected function beforeAction($action)
	{
		...
		//處理單張圖片
		 $tmp = $this->def['updatefield']['sections'][0]['field']['pic1'];
         unset($this->def['updatefield']['sections'][0]['field']['pic1']);
         $this->def['updatefield']['sections'][0]['field']['pic1'] = $tmp;
	...
	public function actionIndex($param='')
	{
		if(empty($_POST)){
			...
			//處理單張圖片
			 $k = 'bannersingle_'.$this->data['admin_switch_data_ml_key'];
             if(isset($sys_configs[$k])){
				$this->data['updatecontent']['pic1'] = $sys_configs[$k];
             }
			...
			if(count($save) > 0){
				...
				//處理單張圖片
				 $k = 'sys_config_bannersingle_'.$this->data['admin_switch_data_ml_key'];
                 $v = $save['pic1'];
                 if(isset($this->data['sys_configs'][$k]) and $v != $this->data['sys_configs'][$k] ){
					$update = $this->update_run_pic(array('pic1'=>$save['pic1']), array('pic1'=>$this->data['sys_configs'][$k]));
					$save[$k] = $update['pic1'];
                 } else {
                    $savedata = $this->create_run_pic(array('pic1'=>$save['pic1']));
                     $save[$k] = $savedata['pic1'];
                 }
