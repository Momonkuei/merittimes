	...
	protected function beforeAction($action)
	{
		...
		if(1){
			$this->data['rowg_custom'] = array(
				...
				'other25' => 'userblockv4index', // [節點] 父節點 router class 名稱
				'other27' => 1, // [節點] 是否為通用
		...
		//echo $session_node['value'];die;
		// 把listfield和updatefield裡面的欄位，跟隨著layoutv3view裡面的欄位做變化
		if(isset($session_node['value']) and $session_node['value'] > 0){
			$aaa = $this->cidb->where('id', $session_node['value'])->get('html')->row_array();
			if($aaa and preg_match('/\%(.*)$/',$aaa['topic'],$matches)){
				$bbb = $this->cidb->where('type','layoutv3view')->where('topic', $matches[1])->get('html')->row_array();
				if(trim($bbb['detail']) != ''){
					unset($ddd);
					$cccs = explode("\n",$bbb['detail']);
					$cccs[0] = '$ddd = array(';
					...
					@eval($ccc);
					if(isset($ddd)){
						$listfield_input_field_key = '';
						$listfield_input_field_name = '';
						foreach($ddd as $k => $v){
							if($v['type'] == 'input'){
								$listfield_input_field_key = $k;
								$listfield_input_field_name = $v['label'];
								break;
							}
						}
						$this->def['listfield'][$k] = array(
							'label' => $v['label'],
							'width' => '50',
						);
						$this->def['updatefield']['sections'][0]['field'] = $ddd;
					}
				}
			}
		}
