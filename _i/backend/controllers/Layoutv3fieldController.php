...
$('#end_date').datepicker({dateFormat: 'yy-mm-dd'});
	...
	protected function beforeAction($action)
	{
		...
		if(isset($_GET['id']) and $_GET['id'] > 0){
			$ss = 'layoutv3field_search';
			$session = Yii::app()->session[$ss];
			if($session === null){
				$session = array();
			}
			$session['class_id'] = $_GET['id'];
			Yii::app()->session[$ss] = $session;

			$this->redirect($this->createUrl($this->data['router_class'].'/index'));
		}

		if(isset($_SESSION['layoutv3field_search']['class_id']) and $_SESSION['layoutv3field_search']['class_id'] > 0){
...
';
		...
		if(1){
			$rowg = array(
				...
				'pic3' => 0, // 日期排序
				'other18' => 1, // 大分類是否可選
				'other22' => 'layoutv3pagetype', // 自定分類名稱
			);
		}
		...
		if($rowg){ 
			...
			if(isset($rowg['pic2']) and $rowg['pic2'] == 1){ // 有分類
				...
				if(isset($rowg['is_news']) and $rowg['is_news'] == 1){ // 是通用分類
					...
					$rows = $this->db->createCommand()->from($producttype_table)->where('is_enable=1 and pid=0')->order('sort_id asc')->queryAll();
					if($rows and count($rows) > 0){
						foreach($rows as $k => $v){
							...
							if(isset($_SESSION['layoutv3field_search']['class_id']) and $_SESSION['layoutv3field_search']['class_id'] > 0){
								$this->def['searchfield']['sections'][0]['field']['class_id']['other']['values'][$_SESSION['layoutv3field_search']['class_id']] = 'XXX';
								$this->def['updatefield']['sections'][0]['field']['class_id']['other']['values'][$_SESSION['layoutv3field_search']['class_id']] = 'XXX';
							}
						}
					}
				}
			...
			foreach($this->data['listcontent'] as $k => $v){
				...
				if($show_type_mode > 0 and $v['class_id'] > 0){
					...
					if($show_type_mode == 1){
						...
						$row2 = $this->db->createCommand()->from($type_name)->where('is_enable=1 and id='.$v['class_id'], array(':ml_key'=>$this->data['admin_switch_data_ml_key']))->queryRow();
						$v['xx2'] = $row2['name'];
					}
	...
	protected function update_show_last($param='')
	{
		...
		// 客製
		$this->data['updatecontent']['name_inputselect'] = array();
		$this->data['updatecontent']['name_inputselect']['ggg1'] = '== 其它 ==';
		foreach(array(
			'__serial_number1' => '流水號 1, 2, 3, ...',
			'__serial_number2' => '流水號 01, 02, 03, ...',
			) as $v){
			$this->data['updatecontent']['name_inputselect']['ggg1_'.$v] = $v;
		}

		$this->data['updatecontent']['name_inputselect']['ggg2'] = '';

		$this->data['updatecontent']['name_inputselect']['ggg3'] = '== 通用資料表 ==';
		foreach($this->cidb->list_fields('html') as $v){
			$this->data['updatecontent']['name_inputselect']['ggg1_'.$v] = $v;
		}

		$this->data['updatecontent']['name_inputselect']['ggg4'] = '';

		$this->data['updatecontent']['name_inputselect']['ggg5'] = '== 獨立資料表 ==';
		foreach($this->cidb->list_fields('product') as $v){
			$this->data['updatecontent']['name_inputselect']['ggg2_'.$v] = $v;
		}
	}
	...
	{		
		for($i=0;$i<$k;$i++)
			$tt = '　'.$tt;
		$rows = $this->db->createCommand()->from($table)->where('is_enable=1 and pid='.$id.'')->order('sort_id')->queryAll();
		if($rows and count($rows) > 0){
			foreach($rows as $k => $v){						
				$data[$v['id']] = $tt.$v['name'];
