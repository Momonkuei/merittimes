 ...
 * 另外想把浮水印的功能加進來
 *
 * timthumb.php?src=圖片位置&h=圖片高度&w=圖片寬度&zc=1
 */
...
// 李哥說，要改成20M，因為有竹的關係，所以設這個新的數值為預設值，原有是10M 2017-10-03
...
if(! defined('DEFAULT_Q') )                             define ('DEFAULT_Q', 100); // 這裡從90改成100                                                                       // Default image quality. Allows overrid in timthumb-config.php
...
// #35225
if(preg_match('/^base64:(.*)$/', $cachefile_tmp, $matches)){
	$cachefile_tmp = $_GET['src'] = str_replace('/assets','assets',base64_decode(urldecode($matches[1])));
	$_GET['nowatermark'] = 1;
} elseif($cachefile_tmp == 'md5update'){
	@unlink('assets/md5update.php');
	$tmps = _getFilesFromDir('assets/members');
	//var_dump($tmps);
	$tmps1 = array();
	$tmps2 = array();
	...
	file_put_contents('assets/md5update.php','<'.'?'.'php $md5update1 = '.var_export($tmps1,true).';$md5update2 = '.var_export($tmps2,true).';');
	die;
} elseif(preg_match('/^md5:(.*)$/', $cachefile_tmp, $matches)){
	if(!file_exists('assets/md5update.php')){
		die;
	}
	include 'assets/md5update.php';
	if(isset($tmps1[$matches[1]])){
		$cachefile_tmp = $_GET['src'] = $tmps1[$matches[1]];
	}
}
...
if(file_exists($cachefile) and !isset($_GET['nocache'])){
...
if(isset($_GET['nowatermark']) and $_GET['nowatermark'] == '1'){
	timthumb::start();
} else {
	$timthumb_argv = timthumb::startg();
...
class timthumb {
        ...
        public function rung(){
                if($this->isURL){
                        ...
                        $this->debug(3, "Got request for internal image. Starting serveInternalImage()");
                        return $this->serveInternalImageg();
                }
                return true;
        }
        ...
        protected function serveInternalImageg(){
                $this->debug(3, "Local image path is $this->localImage");
                if(! $this->localImage){
                        $this->sanityFail("localImage not set after verifying it earlier in the code.");
                        return false;
...
// START - Added for coordinate Crop **********************
                if (!$sData)
                {
...
// END - Added for coordinate Crop **********************      
				...
				$percentage = (double) abs ($this->param('p', 0));
...
// START - Added for coordinate Crop **********************    
                $x1 = (int) abs ($this->param('x1', 0));
                $y1 = (int) abs ($this->param('y1', 0));
                $x2 = (int) abs ($this->param('x2', 0));
                $y2 = (int) abs ($this->param('y2', 0));

				// /var/www/html/mr_lian/upload/product/photo_l_139831629334.jpg
				// echo $localImage;

				// $crop_common_config = array();
				// if(file_exists('images/watermark/crop_common.php')){
				// 	require 'images/watermark/crop_common.php';
				// }

				// $tmps = explode('/', $localImage);
				// $img = $tmps[count($tmps)-1];
				// if(isset($crop_common_config[$img][$this->param('w').'_'.$this->param('h')])){
				// 	$tmp1 = $crop_common_config[$img][$this->param('w').'_'.$this->param('h')];
				// 	$x1 = $tmp1[0];
				// 	$y1 = $tmp1[1];
				// 	$x2 = $tmp1[2];
				// 	$y2 = $tmp1[3];
                ...
                // force to square? (方的)
                $square = $this->param('sq', FALSE);                                            
               
                // Use coordinate cropping? 這裡應該要改成OR，其實不用，前兩個條件有等於，請注意看
                $coord_crop = ($x1 >= 0 && $y1 >= 0 && $x2 > 0 && $y2 > 0) ? TRUE : FALSE;
                ...
                // set new w and h, and adjust based on coordinate crop
                // if coord crop, we'll use coordinates to calc new w/h values          
                $new_width = $coord_crop ? 0 : $width;
                $new_height = $coord_crop ? 0 : $height;

                // if w or h set, we'll use as maximum image size
                $new_width = (int) abs ($this->param('w', $new_width));
                $new_height = (int) abs ($this->param('h', $new_height));

                // new w / h can't exceed defined maximums
                $new_width =  min ($new_width, MAX_WIDTH);
                $new_height = min ($new_height, MAX_HEIGHT);
               
                // if coordinate crop isn't used and w or h not set, we want either proportional or full-size h/w values
                if (!$coord_crop)
                {      
                        if ($new_width == 0)
                        {       // no h ? set to full-size : proportional
                                $new_width = $new_height == 0 ? $width : floor ($width * ($new_height / $height));
                                if($new_height == 0){ $new_height = $height; }
                        } elseif($new_height == 0) {    // w but no h, make proportional
                                $new_height = floor ($height * ($new_width / $width));
                        }
                } else {                
                        // adjust x/y corridinates
                        $x2 = $x2 > $width ? $width : $x2;
                        $y2 = $y2 > $height ? $height : $y2;
                        $x1 = $x1 > $x2 ? floor($x2/2) : $x1;
                        $y1 = $y1 > $y2 ? floor($y2/2) : $y1;
                        ...
                        // if w or h is set, use as max measurement for final cropped image
                        $new_width = $new_width > $new_height ? $new_width : $new_height;
                        $new_height = $new_height > $new_width ? $new_height : $new_width;
                        $new_width = $new_width==0 ? $src_w : $new_width;
                        $new_height = $new_height==0 ? $src_h : $new_height;

                        // maintain aspect ratio of original image/crop
                        if (($src_h / (float)$new_height) > ($src_w / (float)$new_width))
                        {
                                $new_width = floor($new_height * $src_w / (float)$src_h);
                        } else {
                                $new_height = floor($new_width * $src_h / (float)$src_w);
                        }
                }
...
// END - Added for coordinate Crop **********************
                ...
                if ($square)
                {       // equalize side measurements before creating the new image            
                        $new_width = $new_height = min($new_width, $new_height, MAX_WIDTH, MAX_HEIGHT);         // set to shortest side, or smallest MAX                
                }
            ...
            // If is a png and PNG_IS_TRANSPARENT is false then remove the alpha transparency
                ...
                if ($square)
                {       // equalize side measurements before creating the new image            
                        $src_w = $src_h = min($src_w, $src_h);                                                                                          // set to shortest side
				...
				// jerry 2017-02-17
				if($this->dest != ''){
					$tmp = $this->docRoot.'/_i/'.$this->dest;
					$tmps = explode('/', $tmp);
					unset($tmps[count($tmps)-1]);
					@mkdir(implode('/',$tmps),0777,true);
				}
                ...
                if(preg_match('/^image\/(?:jpg|jpeg)$/i', $mimeType)){
						...
						// jerry 2017-02-17
						if($this->dest != '' and !file_exists($this->docRoot.'/_i/'.$this->dest)){
							imagejpeg($canvas, $this->docRoot.'/_i/'.$this->dest, $quality);
						}
						...
						// jerry 2017-02-17
						if($this->dest != '' and !file_exists($this->docRoot.'/_i/'.$this->dest)){
							imagepng($canvas, $this->docRoot.'/_i/'.$this->dest, floor($quality * 0.09));
						}
						...
						// jerry 2017-02-17
						if($this->dest != '' and !file_exists($this->docRoot.'/_i/'.$this->dest)){
							imagegif($canvas, $this->docRoot.'/_i/'.$this->dest);
						}
                } else {
