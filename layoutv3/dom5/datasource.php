...
if(preg_match('/^(.*)\:(.*|)$/', $layer_data_source, $matches)){
	$table = $matches[1];
	$condition = $matches[2];
	$can_append_url_router_method = ''; // 2019-12-10
	...
	if($table == 'webmenu'){
		...
		$can_append_url_router_method = $condition;
		...
		if($this->cidb->table_exists($table)){
			...
			$can_append_url_router_method = $table;
		}
	}
...
if($can_append_url_router_method != '' and !isset($single_content['id'])){ // 不處理單筆的部份，雖然單筆也不會經過這個程序
	$router_method = $can_append_url_router_method;
	...
	if(preg_match('/type$/', $can_append_url_router_method)){
		// SEO 分類
		$seos = $this->db->createCommand()->from('seo')->where('seo_ml_key =:ml_key and seo_type=:type',array(':ml_key'=>$this->data['ml_key'],':type'=>$can_append_url_router_method))->queryAll();
		$seos_type_tmp = array();
		if($seos){
			foreach($seos as $k => $v){
			...
			foreach($single_content as $k => $v){
				if(!isset($v['url']) and isset($v['id'])){
					if(isset($seos_type_tmp[$v['id']]) and $seos_type_tmp[$v['id']]['seo_script_name'] != '' and $_constant === true){
						$single_content[$k]['url'] = $url_prefix.$seos_type_tmp[$v['id']]['seo_script_name'].'.html';
					} else {
						$single_content[$k]['url'] = $router_method.$url_suffix.'?id='.$v['id'];
					}
				}
			}
		}
		...
		if($common_has_page_detail === true){
				...
				foreach($single_content as $k => $v){
					if(!isset($v['url']) and isset($v['id'])){
						if(isset($seos_tmp[$v['id']]) and $seos_tmp[$v['id']]['seo_script_name'] != '' and $_constant === true){
							$single_content[$k]['url'] = $url_prefix.$seos_tmp[$v['id']]['seo_script_name'].'.html';
						} else {
							$single_content[$k]['url'] = $router_method.'detail'.$url_suffix.'?id='.$v['id'];
						}
					}
					// 因為單筆的沒有pid欄位，所以不會經過那個attr2的遞迴程函式
					if(!isset($v['attr2']) and isset($v['id'])){
						if(isset($seos_tmp[$v['id']]) and $seos_tmp[$v['id']]['seo_script_name'] != '' and $_constant === true){
							$single_content[$k]['attr2'] = ' href="'.$url_prefix.$seos_tmp[$v['id']]['seo_script_name'].'.html" ';
						} else {
							$single_content[$k]['attr2'] = ' href="'.$router_method.'detail'.$url_suffix.'?id='.$v['id'].'" ';
						}
					}
				}
			}
			...
			if($single_content){
				foreach($single_content as $k => $v){
					if(!isset($v['url']) and isset($v['id'])){
						$single_content[$k]['url'] = '#';
					}
					if(!isset($v['attr2']) and isset($v['id'])){
						$single_content[$k]['attr2'] = ' href="#" ';
					}
				}
			}
		}
...
if(is_array($single_content) and !empty($single_content) and isset($table)){
	foreach($single_content as $kk => $vv){
		...
		foreach($vv as $kkk => $vvv){
			if(preg_match('/^pic(\d+)$/', $kkk) and $vvv != '' and !preg_match('/^(id)$/', $table)){
				...
				$vv[$kkk.'_'] = $vvv;
			} elseif(preg_match('/^file(\d+)$/', $kkk) and $vvv != '' and !preg_match('/^(id)$/', $table)){
				if($table == 'html'){
		...
		// 2018-12-06
		// $vv['_serial_number'] = (int)$kk+1;

		$vv['__serial_number1'] = '';
		$vv['__serial_number2'] = '';
		if(is_int($kk)){
			$vv['__serial_number1'] = $kk+1; // 1,2,3,4...
			$vv['__serial_number2'] = str_pad(($kk+1),2,'0',STR_PAD_LEFT); // 01,02,03,04...
		}

		// 反正topic和name都是標題，不用煩惱要用哪一個
		if(isset($vv['topic']) and !isset($vv['name'])){
			$vv['name'] = $vv['topic'];
		} elseif(isset($vv['name']) and !isset($vv['topic'])){
			$vv['topic'] = $vv['name'];
		}

		foreach(array('date1','date2','start_date','end_date') as $vvv){
			if(isset($vv[$vvv])){
				if($vv[$vvv] == '0000-00-00'){
					$vv[$vvv] = '';
				}
			}
		}

		foreach(array('create_time','update_time') as $vvv){
			if(isset($vv[$vvv])){
				if($vv[$vvv] == '0000-00-00 00:00:00'){
					$vv[$vvv] = '';
				} else {
					$vv[$vvv] = date('Y/m/d', strtotime($vv[$vvv]));
				}
			}
		}
		...
		foreach($vv as $kkk => $vvv){
			if(preg_match('/^(detail|field_data|field_tmp)$/', $kkk)){
				$newfieldname = $kkk.'___nochange';
				if(!isset($vv[$newfieldname])){
					$vv[$newfieldname] = $vv[$kkk];
				}

				$newfieldname = $kkk.'___nl2br';
				if(!isset($vv[$newfieldname])){
					$vv[$newfieldname] = nl2br($vv[$kkk]);
				}

			} elseif(preg_match('/^(create_time|update_time|date1|date2|start_date|end_date)$/', $kkk)){
				...
				foreach(array('year'=>'Y','month'=>'M','month2'=>'F','MONTH'=>'m','day'=>'d') as $kkkk => $vvvv){
					$newfieldname = $kkk.'___'.$kkkk;
					if(!isset($vv[$newfieldname])){
						$vv[$newfieldname] = ''; // 不管如何，至少弄個空的出來
						if($vv[$kkk] != ''){
							$vv[$newfieldname] = date($vvvv, strtotime($vv[$kkk]));
						}
					}
				}
			}
...
//if(count($single_content) > 0){
if(is_array($single_content) and !empty($single_content)){
	$pidas = 'pid';
	if(isset($m_params['pidas']) and $m_params['pidas'] != ''){
		$pidas = $m_params['pidas'];
	}
