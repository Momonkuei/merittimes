	...
	function renderMenu_dom4_layer($items, $struct, $params = array()) {
		...
		if($items and is_array($items)){
			foreach ($items as $item) {
				...
				if(isset($item['_replace_struct_0']) and $item['_replace_struct_0'] != ''){
					$tmp = $item['_replace_struct_0'];
				}
		...
		if($current_layer_data != ''){
			return "\n".$struct[1]."\n".$current_layer_data.$struct[2]."\n";
		} else {
			return;
		}
	}
...
if(0 and isset($v1) and !empty($v1)){
	foreach($v1 as $kgg => $vgg){
		if(isset($vgg['type'])){
			if($vgg['type'] == 'l'){
				...
				$layer_data_source_id = $vgg['data_source_id']; // 例product:
				...
				// 2018-05-24 新功能開發中
				$layer_struct_a = $vgg['struct_a']; // 1-3|style|{/describe/}|replace
...
// 無限層
for($xgg=0;$xgg<=12;$xgg++){ // 因為裡面所引用的程式碼很深，所以這裡最頂層的變數撰寫要小心
...
// 移動(處理程序一)
// 2018-03-13 這個流程要放在下面一點，比較不會去影響到其它規則 //2020-12-24 因購物列表要多處理 由8改20 2020-02-26 以茶米國為例，應該要到70
for($xgg=0;$xgg<=70;$xgg++){

	// 檢查來源規則(m)是否存在
	$move_parent = 'find("*[m*=]", "'.$xgg.'")';
	// $move_parent = 'find("*[m*=]", "0")';
	unset($target);
	...
	$run = '$move_data_content = $html->'.$move_parent.'->outertext;';
	//echo $run;
	@eval($run); // gg
	...
	// dry hack
	// if($move_data_content == ''){
	// 	$move_content = 'find("*[m*='.$target.']", "0")';
	...
	//echo $move_data_content.'gg';

	// 2018-03-13 因為移動的程序二，會將該區塊的html object，變成text，所以這裡要先把屬性刪掉，不然最下面刪它不掉
	// $html_tmp = str_get_html($move_data_content, true, true, DEFAULT_TARGET_CHARSET, false);
	// $run = '$html_tmp->'.$move_content.'->removeAttribute("m");';
	// eval($run);
	// $move_data_content = $html_tmp->save();
	// echo $move_data_content;

	//$run = '$html->'.$move_parent.'->removeAttribute("m");';

	// 在移動前，先把來源的區塊刪掉
	$run = '$html->'.$move_parent.'->outertext = "";';
	eval($run);

	// 把東西移到mg裡面，然後在程序二的時候，會把mg的外層刪掉
...
if($has_rule === true){
	...
	// 移動
	for($x=0;$x<=70;$x++){ //2020-12-24 因購物列表處理比較多 由8改20 2020-02-26 以茶米國為例，應該要到70
		// $parent = 'find("*[t*=]", "'.$x.'")'; // 因為下面移除掉了，所以如果要移除的話，就是移除第一個
		$parent = 'find("*[m*=]", "0")';
	...
	for($x=0;$x<=8;$x++){
		...
		unset($function_list_tmp);
		$run = '$function_list_tmp = $html->'.$parent.'->k;';
		...
		unset($function_list_tmp2);
		$run = '$function_list_tmp2 = $html->'.$parent.'->innertext;';
		eval($run);

		$run = '$html->'.$parent.'->outertext="";';
		eval($run);

		$this->data['dom5_kg_'.$function_list_tmp] = $function_list_tmp2;
	}
	...
	$aaa = str_replace('id="_dom5_layer_pagenav_"', '', $aaa);
	$aaa = str_replace('class="_dom5_layer_pagenav_"', '', $aaa);

	// 2018-10-30 A方案的特殊選單範例 (三大有線電視)
	// $aaa = str_replace('<ulgg >', '<div class="navSubMenuBox"><div class="triangleTail"></div><ul class="navSubMenu">', $aaa);
	// $aaa = str_replace('</ulgg>', '</ul></div>', $aaa);
	// $aaa = str_replace(' ggxx>', ' ><li class="mobileMenu-Back"><span class="icon"></span><span class="text">Menu</span></li>', $aaa);
	// $aaa = str_replace(' abcdefg>', ' ><tr><th>區域</th><th>店名</th><th>電話</th><th>地址</th></tr>', $aaa);
	// $aaa = str_replace(' gggzzz>', ' ><tr><th>Document</th><th>Description</th><th>Download</th></tr>', $aaa);

	// 2018-12-12
	if(isset($this) and isset($this->data) and !empty($this->data)){
		foreach($this->data as $k => $v){
			if(preg_match('/^dom5_kg_(.*)$/', $k, $matches)){
				// 開頭
				$aaa = str_replace(' kg="'.$matches[1].'">', ' >'.$v, $aaa);

				// 結尾 2020-09-25
				foreach(array('ul','div') as $vv){
					$aaa = str_replace('</'.$vv.'><span>'.$matches[1].'</span>', $v.'</'.$vv.'>', $aaa);
				}
			}
		}
	}
	...
	// 2018-09-13
	$aaa = str_replace('m="head_start"', '', $aaa);
	$aaa = str_replace('m="head_end"', '', $aaa);
	$aaa = str_replace('m="body_start"', '', $aaa);
	$aaa = str_replace('m="body_end"', '', $aaa);
	$aaa = str_replace('m="form_post"', '', $aaa);
