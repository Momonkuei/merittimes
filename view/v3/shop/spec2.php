...
<!-- // DATA2:MULTI -->
		<?php if(isset($data[$ID])):?>
			<?php foreach($data[$ID] as $k => $v):?>
				<label><input type="radio" class="<?php echo $ID?>" name="<?php echo $item['name']?>" value="<?php echo $v['value']?>"  <?php if(isset($v['disabled'])):?> disabled="disabled" <?php endif?> >  <span><?php if(isset($v['pic'])):?><img src="<?php echo $v['pic']?>" /><?php endif?> <?php echo $v['name']?></span> </label>
			<?php endforeach?>
		<?php endif?>
	</div>
</div>
