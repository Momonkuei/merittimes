<?php if(isset($data[$ID])):?>
	...
	<div class="prod_blk">
	  <div class="row prod_slider">
			...
			<div class="slider-for">
				<?php if(isset($images_small)):?>
					<?php foreach($images_small as $k => $v):?>
						<?php if($v['pic']!=''):?>
						<div>
							<?php //如果不要lightbox拿掉item-image?>
							<div class="<?php echo $data['image_ratio'];//變數在source/core.php?>  item-image"><img src="<?php echo $v['pic']?>" alt="<?php echo $v['name']?>"></div>
						</div>
					<?php endif?>
					<?php endforeach?>
				<?php endif?>
		...
		<div class="prod_info">
			<div class="prodInfo_title"><?php //echo t('特色');//#42470 ming說不需要顯示特色這兩個字 2021-11-12?></div>
			<div class="editorBlock"><?php echo nl2br($product['detail']);//#42470?></div>
		</div><!-- .prod_info -->
		<?php endif?>
	...
	<?php /* if( (isset($product['content1']) and $product['content1']) != '' or (isset($product['content2']) and $product['content2'] != '') )://#31796?>	
		...
		<?php if(isset($product['content1']) and $product['content1']!='')://#31796?>
			<?php $_active = 'active';?>
			<a href="#_" class="tabLabel <?php echo $_active?>"><?php echo t('商品說明')?></a>
			<div class="tabContent">
				<div class="editorBlock">
		...
		<?php if(isset($product['content2']) and $product['content2']!='')://#31796?>
			...
			<a href="#_" class="tabLabel <?php echo $_active?>"><?php echo t('商品規格')?></a>
			<div class="tabContent">
				<div class="editorBlock">
	...
	<?unset($_constant);
		...
		if($_constant){
			$content_list=$this->cidb->where('ml_key',$this->data['ml_key'])->where('class_id',$_GET['id'])->where('type','productcontent')->where('is_enable',1)->order_by('sort_id')->get('html')->result_array();
			if(!empty($content_list)){
			?>
				<div class="container-fluid prd-detail-detail7 bg-line">		
					<?foreach($content_list as $k => $v){?>	
						<div class="container">
							<div class="innerBlock innerBlock_detail7 ">
								<div class="row">
									<div class="col-12 col-lg-6">
										<div class="innerBlock_detail7_txt v4_animate fadeUp delay_03">
											<p><?=$v['topic']?></p>
										</div>

									</div>
									<div class="col-12 col-lg-6">
										<div class="img-rectangle img-scrollBox">
											<img class="v4_animate fadeLeft delay_05" src="/_i/assets/upload/productcontent/<?=$v['pic1']?>" alt="">
										</div>
									</div>
								</div>
							</div>
