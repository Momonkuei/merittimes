...
if(!function_exists('cowboy_menu_tree')){
	function cowboy_menu_tree($data,$floor=0,$str=''){
		...
		if(is_array($data['submenu']) && $floor == 0){ //設計只給兩層
			$str .='<ul class="submenu">'."\r\n";
			
			foreach ($data['submenu'] as $key => $value) {
				$str .= cowboy_menu_tree($value,($floor+1));
			}
			$str .='</ul>'."\r\n";
		}	
...
<div class="cowboy_mbPanel_2" <?php //style="display: block !important;"?>>
