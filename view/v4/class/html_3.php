<?
include _BASEPATH . '/../source/class/post_3.php';
?>
            ...
            <div class="application-form-details billboard-form-details">
                <div class="top-function-column">
                    <div class='add-area'>
                        ...
                        <a href="class_tw_9.php" class='btn-register btn-text'>
                            新增
                            ...
                            <? if (!empty($pic_data)) {
                                foreach ($pic_data as $k => $v) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <a href="class_<?= $this->data['ml_key'] ?>_9.php?pic_id=<?= $v['id'] ?>" class="btn-revise btn-text">
                                                    修改
                                                </a>
                                                <a class="btn-delete btn-text pic_delete" data-cid="<?= $v['id'] ?>">
                                                    刪除
                                                </a>
                                            </div>
                                        </td>
...
<script m="body_end">
    ...
    clear_b.addEventListener('click', function(e) {
        window.location.href = '/class_<?= $this->data['ml_key'] ?>_3.php';
    }, false);
    //個別班級刪除
    var pic_delete = document.querySelectorAll('.pic_delete');
    for (let i = 0; i < pic_delete.length; i++) {
        pic_delete[i].addEventListener("click", function(e) {
            if (confirm("是否要刪除?") == true) {
                let id = pic_delete[i].dataset.cid;
                $.ajax({
                    type: "POST",
                    data: {
                        'type': 'del_pic',
