<?
include _BASEPATH . '/../source/class/post_4.php';
?>
            ...
            <div class="application-form-details billboard-form-details">
                <div class="top-function-column">
                    <div class='add-area'>
                        ...
                        <a href="class_tw_10.php" class='btn-register btn-text'>
                            新增
                            ...
                            <? if (!empty($vido_data)) {
                                foreach ($vido_data as $k => $v) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="btn-group">
                                                <a href="class_<?= $this->data['ml_key'] ?>_10.php?vido_id=<?= $v['id'] ?>" class="btn-revise btn-text">
                                                    修改
                                                </a>
                                                <a class="btn-delete btn-text vido_delete" data-cid="<?= $v['id'] ?>">
                                                    刪除
                                                </a>
                                            </div>
                                        </td>
...
<script m="body_end">
    ...
    //個別班級刪除
    var vido_delete = document.querySelectorAll('.vido_delete');
    for (let i = 0; i < vido_delete.length; i++) {
        vido_delete[i].addEventListener("click", function(e) {
            if (confirm("是否要刪除?") == true) {
                let id = vido_delete[i].dataset.cid;
                $.ajax({
                    type: "POST",
                    data: {
                        'type': 'del_vido',
