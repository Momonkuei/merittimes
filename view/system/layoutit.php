...
function aaa($items, $demo_pic_tmp = array(), $cidb = null){
	$render = '';
	// var_dump($items);
	if($items and count($items) > 0){
		foreach ($items as $k => $item){
			...
			$item['params_array'] = array();
			if($item['params'] != ''){
				//$item['params_array'] = json_decode('{'.$item['params'].'}',true);

				$layer_params = $item['params'];
				...
				$item['params_array'] = json_decode('{'.implode(',', $m_params2).'}', true);
			}
			...
			$render .= '			<div class="row clearfix">'."\n"; // 2020-01-14 ruby說要加這個，左右的時候才不會亂掉

			$phy_file = _BASEPATH.'/../view/'.$file.'.php';
			$tmp = '';
			if(preg_match('/^(-group)$/', $file)){ // 只有一個洞，只能擺一個
				...
				if($demo_pic_tmp and isset($demo_pic_tmp[$file])){
					...
					// 2020-01-14
					// 函式放在layoutv3/libs.php
					$return = group_has_hole($file, $cidb);


					if($return and isset($return['count']) and $return['count'] > 0){
						if(isset($item['params_array']['table_horizontal']) and $item['params_array']['table_horizontal'] == '1'){
							$_SESSION['layoutit_aaa']['column column_'.$item['parent_id'].'_'.$item['id']] = $return['count'];
						}
						for($x=1;$x<=$return['count'];$x++){
							$tmp .= '<div class="column column_'.$item['parent_id'].'_'.$item['id'].'"> </div_'.$item['parent_id'].'_'.$item['id'].'>'."\n";
						}
					} else {
						$tmp .= '群組：{ '.$file.' }'."\n";
				...
				if($demo_pic_tmp and isset($demo_pic_tmp[$file])){
					...
					foreach(array('AA','BB','CC','DD','EE') as $vv){
						$tmp = str_replace('<'.'?'.'php echo $'.$vv.'?'.'>','<div class="column column_'.$item['parent_id'].'_'.$item['id'].'"> </div_'.$item['parent_id'].'_'.$item['id'].'>'."\n",$tmp);
					}
					...
					if(preg_match('/\<\?php/', $tmp)){
						if(!preg_match('/column\ column_/',$tmp)){
							if(!isset($_SESSION['layoutit_showphp'])){
								$tmp = '含程式：{ '.$file.' }'."\n";
							}
						} else {
							// 2020-01-14 有PHP，而且沒有洞的區塊，就執行它，取沒有PHP的內容
							if(!isset($_SESSION['layoutit_showphp'])){
								ob_start();
									@eval('?'.'>'.$tmp);
								$tmp = ob_get_contents();
			...
			if (!empty($item['hole'])) {
				...
				if($tmp != ''){
					if($item['file'] == '-group'){ // 可以擺多個
						...
						$render .= '		' .aaa($item['hole'], $demo_pic_tmp, $cidb);
						...
						$tmp2 = aaa($item['hole'], $demo_pic_tmp, $cidb);
			...
			$render .= '			</div>'."\n"; // row 2020-01-14 ruby說要加這一層，左右的時候才不會亂掉
			$render .= '		</div>'."\n"; // view
...
$demo = aaa($page_layoutit, $demo_pic_tmp, $this->cidb);

// 2020-01-14 試著加上col-md-X在column裡面
// var_dump($_SESSION['layoutit_aaa']);
// column column_X_X => hole Num
if(isset($_SESSION['layoutit_aaa']) and !empty($_SESSION['layoutit_aaa'])){
	foreach($_SESSION['layoutit_aaa'] as $k => $v){
		$a = floor(12/$v); // 取除數
		$b = 12%$v; // 取除數
		...
		for($x=1;$x<=$v;$x++){
			...
			$tmps = explode(' ', $k);
			$demo = str_replace_first($k,$tmps[0].' col-md-'.$num.' '.$tmps[1],$demo);
		}
	}
}
