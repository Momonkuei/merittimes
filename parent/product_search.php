...
/*
 * 2020-10-21
 * 這個是V4專用的，而且是要有提升分類的情況下，然後又有搜尋的狀況在使用的
 */
					...
					'hole' => array(
						...
						array(
							...
							'hole' => array(
								array(
									'hole' => array(
										array('file' => 'v4/product/list1_search'),
...
if(preg_match('/_/', $this->data['router_method']) and isset($layoutv3_struct_map_keyname['v3/breadcrumb']) and isset($data[$layoutv3_struct_map_keyname['v3/breadcrumb'][0]][2])){
...
if(preg_match('/^member(center|bonuslist|orderlist|orderdetail|forget|forgetconfirm|changepassword|customeraddress|noticepayment)$/', $this->data['router_method'])){
	...
	if($this->data['ml_key'] == 'tw'){
		$member_page_title = array("name"=>'收藏清單',"sub_name"=>'favorite list');
		$member_breadcrumb = array(array("name"=>"HOME","url"=>"/"),array("name"=>'收藏清單',"url"=>'favorite_tw.php'));
	} else {
		$member_page_title = array("name"=>'Favorite List',"sub_name"=>'');
		$member_breadcrumb = array(array("name"=>"HOME","url"=>"/"),array("name"=>'Favorite List',"url"=>'favorite_en.php'));
	}
