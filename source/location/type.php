...
<?php
		...
		if(isset($_GET['id'])){
			...
			if($tmp and isset($tmp) and isset($tmp['id'])){
				...
				$url = $url_prefix.$this->data['router_method'].$url_suffix.'.php?page=';
				$qryField = '*';
				...
				if($db->total_row > 0) {
					...
					do{
						...
						$db->row['address_url'] = $db->row['detail'];
						$db->row['address'] = $db->row['url1'].'<i class="fa fa-map-marker"></i>';
						$db->row['phone'] = $db->row['other1'];
						$db->row['fax'] = $db->row['other2'];
						$db->row['email_url'] = 'mailto:'.$db->row['other3'];
						$db->row['email'] = $db->row['other3'];
						$db->row['website_url'] = '';
						$db->row['website'] = '';

						if(isset($admin_field['pic1']['other']['width'])){
							if($admin_field['pic1']['other']['width'] == '40'){
								$db->row['pic_small'] = '_i/assets/upload/'.$this->data['router_method'].'/'.$db->row['pic1'];
							} elseif($admin_field['pic1']['other']['width'] == '800'){
								$db->row['pic_big'] = '_i/assets/upload/'.$this->data['router_method'].'/'.$db->row['pic1'];
							}
						}
