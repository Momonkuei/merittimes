...
<?php
	...
	if(defined('LAYOUTV3_THEME_NAME') && LAYOUTV3_THEME_NAME == 'v4'){
		...
		$tmp['right_list'] = $this->cidb->where('is_enable',1)->where('(reserve_date <= now() OR reserve_date IS NULL OR reserve_date = "0000-00-00")')->where('(retirement_date > now() OR retirement_date IS NULL OR retirement_date = "0000-00-00")')->where('ml_key',$this->data['ml_key'])->where('type',str_replace('detail','',$this->data['router_method']))->where('id !=',$tmp['id'])->order_by('sort_id')->limit(10)->get('html')->result_array();	
	}
} else { // 無分類
