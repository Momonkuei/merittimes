...
$qryField = '*';
// 2018-03-13 預設抓第一個多圖上傳區塊的資料
$qryWhere = ' WHERE type=\''.$this->data['router_method'].'tmp1\' and class_id='.$_GET['id'].' order by id asc  ';//按照id排序列出 by lota	
...
if($db->total_row > 0) {
	...
	do{
		...
		// 列表圖
		$db->row['pic'] = cache3('_i/'.$db->row['pic1']);

		// 大圖
		$db->row['url'] = cache3('_i/'.$db->row['pic1']);
...
if(isset($layoutv3_struct_map_keyname['v3/category_title'][0])){
	...
	$aaa = array();
	$path = _BASEPATH.'/assets/members/'.str_replace('detail','',$this->data['router_method']).'_1_'.$_GET['id'].'/member';
	if(is_dir($path)){
