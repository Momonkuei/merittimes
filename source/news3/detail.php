...
$download_list=$this->cidb->where('type','news'.$num_type.'other4')->where('is_enable',1)->where('class_id',$_GET['id'])->order_by('sort_id')->get('html')->result_array();
if(!empty($tmp['other9'])){
	$tmp['other9_pic']=explode('?',$tmp['other9']);
	if(!empty($tmp['other9_pic'][1])){
		$tmp['other9_pic']=str_replace('v=','',$tmp['other9_pic'][1]);
	}
	
}
if(!empty($tmp['other10'])){
	$tmp['other10_pic']=explode('?',$tmp['other10']);
	if(!empty($tmp['other10_pic'][1])){
		$tmp['other10_pic']=str_replace('v=','',$tmp['other10_pic'][1]);
	}	
}
// print_r($tmp);die;
if($download_list and !empty($download_list)){
	foreach($download_list as $k => $v){
		...
		$v['file_size'] = '';
		if($v['file1'] != ''){
			if(is_file('_i/assets/file/news'.$num_type.'other4/'.$v['file1'])){
				$url = '_i/assets/file/news'.$num_type.'other4/'.$v['file1'];
				$num=1;
				$bytes=filesize($url);
