...
if($match and !empty($match)){
	...
	foreach($match as $k => $v){
		if($v['condition'] == '1' and $v['match'] === true){
			...
			$match[$k] = $v;
		} elseif($v['condition'] == '2' and isset($v['handle_1']) and !empty($v['handle_1'])){
			$promotion = $v['promotion'];
...
if(isset($this->data['admin_id']) and $this->data['admin_id'] > 0){
	...
	if(isset($_SESSION['save']['member_form_2']['select_recipient']) and $_SESSION['save']['member_form_2']['select_recipient'] != ''){
		...
		} elseif($_SESSION['save']['member_form_2']['select_recipient'] == 'buyer' && !isset($_GET['go_to_step2']) && !isset($_SESSION['save']['step2']['go_to_step2']) ){ //2020-12-16 加入 GET 變數，在view/checkout/step2.php 841 帶過來的，要確定已經不是重新整理而是送出到下個階段 by lota
			// $recipient = array(
	...
	// 同訂購人 (非會員)
	if(isset($_SESSION['save']['member_form_2']['select_recipient']) and $_SESSION['save']['member_form_2']['select_recipient'] != ''){
		if($_SESSION['save']['member_form_2']['select_recipient'] == 'buyer' && !isset($_GET['go_to_step2']) && !isset($_SESSION['save']['step2']['go_to_step2']) ){ //2020-12-16 加入 GET 變數，在view/checkout/step2.php 841 帶過來的，要確定已經不是重新整理而是送出到下個階段 by lota
			// $recipient = array(
			// 	'recipient_name' => $_SESSION['save']['member_form_1']['name'],
			// 	'recipient_gender' => $_SESSION['save']['member_form_1']['gender'],
			// 	'recipient_phone' => $_SESSION['save']['member_form_1']['phone'],
...
if($_SESSION[$this->data['router_method']]['step'] == 1){
	...
	if(isset($_SESSION['save']['step3']['go_to_finish!!']) and $_SESSION['save']['step3']['go_to_finish!!'] == '1'){
		...
		if(!isset($_SESSION['save']['step3']['order_created_id']) or $_SESSION['save']['step3']['order_created_id'] == ''){ 
			...
			// 2020-06-11 513發現的問題 2020-12-16 加入 recipient_gender 2021-01-04 改變判斷模式 by lota
