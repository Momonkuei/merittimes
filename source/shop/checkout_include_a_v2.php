...
if($promotions){
	foreach($promotions as $k => $v){
		...
		if(isset($v['scope']) && ($v['scope'] == '1' or $v['scope'] == '0')){ //2021-06-06 lota fix
 ...
 */
//var_dump($match);die;
$no_promo_match=array();
$tmp = array(); // 第一個存promotion_id，數值為流水號，從0開始
$tmp_promo = array(); 
if($car and !empty($car)){
	foreach($car as $k => $v){
		//除去滿額加價購產品
		if($v['item']['is_promo']==0){
			...
			$no_promo_match[$v['promotion_id']]['condition'] = '2';
			$no_promo_match[$v['promotion_id']]['promotion'] = $v['item']['promotion'];
			//var_dump($match);

			if(!isset($no_promo_match[$v['promotion_id']]['handle_1'])) $no_promo_match[$v['promotion_id']]['handle_1'] = array();
			if(!isset($tmp_promo[$v['promotion_id']])) $tmp_promo[$v['promotion_id']] = 0;

			for($x=1;$x<=$v['amount'];$x++){
				$_count = 0;
				if(isset($no_promo_match[$v['promotion_id']]['handle_1'][$tmp_promo[$v['promotion_id']]])){
					$_count = count($no_promo_match[$v['promotion_id']]['handle_1'][$tmp_promo[$v['promotion_id']]]);
				}
				if(isset($no_promo_match[$v['promotion_id']]['handle_1'][$tmp_promo[$v['promotion_id']]]) and $_count < $v['promotion_condition_2']){
					// do nothing
				} else {
					$tmp_promo[$v['promotion_id']]++;
				}
				$no_promo_match[$v['promotion_id']]['handle_1'][$tmp_promo[$v['promotion_id']]][] = $k;
			}
		}
		if(isset($v['promotion_condition_1']) and $v['promotion_condition_1'] != '2' and $v['promotion_condition_2'] >= 1) continue; // 這裡只處理有效的滿件哦
