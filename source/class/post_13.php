...
//全部
$vido_list_all=$this->cidb->where('class_id',$_SESSION['member_data']['class_id'])->where('is_enable',1)->where('ml_key',$this->data['ml_key'])->order_by('create_time desc')->get('class_vido')->result_array();
$total_rows = count($vido_list_all);

////單頁
$vido_list=$this->cidb->where('class_id',$_SESSION['member_data']['class_id'])->where('is_enable',1)->where('ml_key',$this->data['ml_key'])->order_by('create_time desc')->get('class_vido', $limit_count, ($pagew - 1) * $limit_count)->result_array();

$url = 'class_'.$this->data['ml_key'].'_12.php?page=';
