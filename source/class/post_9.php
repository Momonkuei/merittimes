...
if(!empty($_GET['pic_id'])){
	$text='修改';
	$_SESSION['class_9']=$this->cidb->where('id',$_GET['pic_id'])->get('class_pic')->row_array();
	$re_url='?pic_id='.$_GET['pic_id'];
}else{
	$text='新增';
	$re_url='';
	unset($_SESSION['class_9']);
}
if(!empty($_POST)){
	//圖片上傳
	$_SESSION['class_9']=$_POST;
			...
			if($v['size']>$upload_max_filesize){
				...
				echo"
					...
					<script>
						...
						window.location.href='/class_".$this->data['ml_key']."_9.php".$re_url."';
					</script>";
				continue;
			}
	...
	if(!empty($_GET['pic_id'])){
		$this->cidb->where('id',$_GET['pic_id']);
		if($this->cidb->update('class_pic',$data_array)){
            $id=1;
        }
	}else{
		$this->cidb->insert('class_pic',$data_array); 
