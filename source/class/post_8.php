...
if(!empty($_GET['billboard_id'])){
	$text='修改';
	$_SESSION['class_8']=$this->cidb->where('id',$_GET['billboard_id'])->get('class_billboard')->row_array();
	$re_url='?billboard_id='.$_GET['billboard_id'];
}else{
	$text='新增';
	$re_url='';
	unset($_SESSION['class_8']);
}
if(!empty($_POST)){
	//圖片上傳
	$_SESSION['class_8']=$_POST;
			...
			if($v['size']>$upload_max_filesize){
				...
				echo"
					...
					<script>
						...
						window.location.href='/class_".$this->data['ml_key']."_8.php".$re_url."';
					</script>";
				continue;
			}
	...
	if(!empty($_GET['billboard_id'])){
		$this->cidb->where('id',$_GET['billboard_id']);
		if($this->cidb->update('class_billboard',$data_array)){
            $id=1;
        }
	}else{
		$this->cidb->insert('class_billboard',$data_array); 
