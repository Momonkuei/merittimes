...
//全部
$pic_list_all=$this->cidb->where('class_id',$_SESSION['member_data']['class_id'])->where('is_enable',1)->where('ml_key',$this->data['ml_key'])->order_by('create_time desc')->get('class_pic')->result_array();
$total_rows = count($pic_list_all);

//單頁
$pic_list=$this->cidb->where('class_id',$_SESSION['member_data']['class_id'])->where('is_enable',1)->where('ml_key',$this->data['ml_key'])->order_by('create_time desc')->get('class_pic', $limit_count, ($pagew - 1) * $limit_count)->result_array();
