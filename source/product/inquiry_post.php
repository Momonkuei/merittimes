...
if(!empty($_POST)){
	...
	if($_SESSION['save'][$router_method] and count($_SESSION['save'][$router_method]) > 0){
		$tmp_message = $savedata['detail'];
		$savedata['detail'] .= "\n\n".'洽詢商品：'."\n";
		foreach($_SESSION['save'][$router_method] as $k => $v){

			// 為了要能夠支援兩個產品線
			if(preg_match('/^(.*)___(.*)$/', $k, $matches)){
				$table = $matches[1];
				$id = $matches[2];
			} else { // 向下支援，雖然目前己經沒有用到了
				$table = str_replace('inquiry', '', $this->data['router_method']);
				$id = $k;
			}

			//$row = $this->db->createCommand()->from('html')->where('is_enable=1 and id='.$id)->queryRow();
			$row = $this->db->createCommand()->from($table)->where('is_enable=1 and id='.$id)->queryRow();
			if($row and isset($row['id'])){ //2017/9/27 修正詢問信件連結 by lota

				if(!isset($row['name']) and isset($row['topic'])){
					$row['name'] = $row['topic'];
				}

				$content1 = "\n\r <a href=\"http://".aaa_url."/".$table."detail_".$this->data['ml_key'].".php?id=".$row['id']."\" target=\"_blank\"><img src=\"/_i/assets/upload/".$table."/".$row['pic1']."\" alt=\"".$row['name']."\" style=\"width: 50px;\">".$row['name'];
				$content2 = "\n\r <a href=\"http://".aaa_url."/".$table."detail_".$this->data['ml_key'].".php?id=".$row['id']."\" target=\"_blank\">".$row['name'];
