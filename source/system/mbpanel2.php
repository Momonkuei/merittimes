...
//echo json_encode(array('mbPanel'=>array();));
		...
		foreach($result as $k => $v){
			...
			if(isset($v['anchor_open']) and trim($v['anchor_open']) != ''){
				...
				if(preg_match('/class\=\'(.*)\'\ data\-target\=\'(.*)\' href=\'(.*)\'/', $v['anchor_open'], $matches)){ // 2020-02-26 漢堡專用
					...
					$v['class'] = $matches[1];
					$v['link'] = ''; // winnie 2017-11-14 下午說的
				} elseif(preg_match('/target\=\'(.*)\'/', $v['anchor_open'], $matches)){ // 2021-03-29
					$v['target2'] = $matches[1];
				}
...
// var_dump($panel);die;
//2021-12-29 如果是政佳的手機板引入的話，就不需要產出JSON格式了 by lota
if(!isset($cowboy_mbPanel)){
	...
	echo trim(json_encode(array('mbPanel'=>$panel)));
	die;
}
